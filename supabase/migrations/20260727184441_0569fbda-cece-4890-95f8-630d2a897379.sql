CREATE OR REPLACE FUNCTION public.rpc_china_criar_projeto_espelho(p_submissao_id uuid, p_projeto_id uuid DEFAULT NULL::uuid, p_template_b2c_id uuid DEFAULT NULL::uuid, p_secao_nome text DEFAULT 'Documentos da Submissão'::text, p_projeto_nome text DEFAULT NULL::text, p_data_inicio date DEFAULT NULL::date, p_data_fim_alvo date DEFAULT NULL::date, p_prazo_padrao_tarefa integer DEFAULT NULL::integer, p_alerta_antecipacao_dias integer DEFAULT NULL::integer, p_regime_calendario text DEFAULT NULL::text, p_usa_feriados boolean DEFAULT NULL::boolean, p_uf_feriados text DEFAULT NULL::text, p_substituir boolean DEFAULT false, p_estrutura jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_already_espelho IS NOT NULL THEN
    IF p_substituir THEN
      ...
      -- Mantém a linha de vínculo (há UNIQUE por submissao_id); o INSERT
      -- adiante faz ON CONFLICT (submissao_id) e reaponta o projeto_id.
  ...
  LOOP
    ...
    -- Resolve o nome real quando o tipo aponta para um item do Cofre
    v_doc_label := NULL;
    IF v_doc.tipo_documento ~ '^cofre_[0-9a-fA-F-]{36}$' THEN
      SELECT c.nome_pt INTO v_doc_label
