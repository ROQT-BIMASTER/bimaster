...
BEGIN
  ...
  IF v_tarefa_id IS NULL THEN
    ...
    IF v_item.responsavel_brasil_id IS NOT NULL THEN
      ...
      -- Garante acesso estável ao projeto-espelho mesmo se a tarefa for reatribuída
      INSERT INTO public.projeto_membros (projeto_id, user_id, papel)
      VALUES (v_projeto_id, v_item.responsavel_brasil_id, 'membro')
...
-- Backfill: insere responsáveis Brasil já vinculados a tarefas espelho
INSERT INTO public.projeto_membros (projeto_id, user_id, papel)
SELECT DISTINCT t.projeto_id, ck.responsavel_brasil_id, 'membro'
FROM public.china_checklist_brasil_china ck
JOIN public.projeto_tarefas t ON t.id = ck.projeto_tarefa_id
JOIN public.projetos p ON p.id = t.projeto_id
WHERE ck.responsavel_brasil_id IS NOT NULL
  AND p.tipo = 'china_submissao'
ON CONFLICT (projeto_id, user_id) DO NOTHING;