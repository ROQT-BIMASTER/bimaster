-- =========================================================================
-- Assinatura eletrônica simples (trilha de auditoria) em aprovações do chat
-- =========================================================================
--
-- FASE 2 da feature "documentos + assinatura em aprovações do chat".
--
-- Modelo: assinatura eletrônica SIMPLES (Lei 14.063/2020) — o ato de aprovar
-- registra uma trilha de auditoria: quem (decidido_por), quando (decidido_em),
-- IP e user-agent (capturados aqui) + o SHA-256 de cada documento
-- (chat_aprovacao_documentos.hash_arquivo, calculado no cliente no upload).
-- NÃO há carimbo no PDF nem provedor externo: signed_storage_path fica NULL;
-- o "comprovante" é renderizado e impresso no cliente.
--
-- Esta migration:
--   1) adiciona colunas de IP/user-agent em chat_aprovacoes;
--   2) recria rpc_chat_aprovacao_decidir mantendo TODA a lógica atual e
--      acrescentando: captura de IP/UA na decisão e, quando 'aprovado',
--      marca os documentos da aprovação como 'assinado'.
...
BEGIN
  ...
  -- valida participação (ou admin — pode decidir via Central de Aprovações)
  ...
  -- trilha de auditoria: IP + user-agent da requisição (assinatura simples)
  ...
  -- quando aprovado, marca os documentos como assinados (trilha resumida)
...
COMMENT ON FUNCTION public.rpc_chat_aprovacao_decidir IS
  'Aprova ou rejeita; cria mensagem sistema de decisao. Em aprovado, registra
  assinatura eletrônica simples: IP/user-agent (trilha de auditoria) e marca os
  documentos da aprovação como assinado.';
