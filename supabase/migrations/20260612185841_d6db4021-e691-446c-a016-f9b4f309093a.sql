DROP FUNCTION IF EXISTS public.rpc_estoque_cores_kpis_consolidado(integer[], text[], uuid[], text, boolean, boolean, text[], text[], boolean);

CREATE OR REPLACE FUNCTION public.rpc_estoque_cores_kpis_consolidado(p_empresas integer[] DEFAULT NULL::integer[], p_linhas text[] DEFAULT NULL::text[], p_campanha_ids uuid[] DEFAULT NULL::uuid[], p_busca text DEFAULT NULL::text, p_apenas_com_saldo boolean DEFAULT false, p_com_pedido_pendente boolean DEFAULT false, p_curva_fisica text[] DEFAULT NULL::text[], p_curva_monetaria text[] DEFAULT NULL::text[], p_incluir_potencial boolean DEFAULT true)
 RETURNS TABLE(total_skus bigint, total_unidades numeric, total_unidades_potencial numeric, total_pedido_pendente numeric, itens_sem_saldo bigint, total_bloqueado_produto numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
