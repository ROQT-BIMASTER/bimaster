...
RETURNS TABLE(
  ...
  projeto_id uuid, projeto_nome text, projeto_cor text, estagio text,
  criador_id uuid, visibilidade text, secao_id uuid, secao_nome text,
  ordem integer, parent_tarefa_id uuid, responsavel_id uuid,
  responsavel_nome text, responsavel_avatar_url text, codigo text,
  produto_id uuid, created_at timestamp with time zone,
  updated_at timestamp with time zone, papel text,
  ticket_id uuid, ticket_protocolo text, ticket_status text,
  ticket_sla_status text, ticket_prazo_resolucao_em timestamp with time zone,
  ticket_fila_id uuid, ticket_fila_nome text,
  ticket_ultima_interacao_em timestamp with time zone,
  ticket_prioridade text, ticket_conversa_id uuid
)
...
AS $function$
  WITH minhas AS (
    ...
    WHERE auth.uid() IS NOT NULL
      ...
      AND NOT EXISTS (
        ...
        WHERE pj.id = t.projeto_id AND pj.deleted_at IS NOT NULL
      )
      AND (
    ...
    SELECT * FROM dedup WHERE status IS DISTINCT FROM 'concluida'
    ...
    FROM dedup d WHERE d.status = 'concluida'
  ), concluidas_selecionadas AS (
    SELECT * FROM concluidas_ranked WHERE p_limite_concluidas IS NULL OR rn <= p_limite_concluidas
  ), final AS (
    SELECT a.id, a.titulo, a.descricao, a.status, a.prioridade, a.data_inicio_planejada, a.data_prazo, a.data_conclusao,
    ...
    FROM ativas a
    UNION ALL
    SELECT c.id, c.titulo, c.descricao, c.status, c.prioridade, c.data_inicio_planejada, c.data_prazo, c.data_conclusao,
  ...
  SELECT
    f.id, f.titulo, f.descricao, f.status, f.prioridade,
    f.data_inicio_planejada, f.data_prazo, f.data_conclusao,
    f.projeto_id, COALESCE(p.nome, 'Sem projeto'), COALESCE(p.cor, '#6366f1'),
    f.estagio, f.criador_id, f.visibilidade, f.secao_id, s.nome,
    COALESCE(f.ordem, 0), f.parent_tarefa_id, f.responsavel_id,
    pr.nome, pr.avatar_url, f.codigo, f.produto_id, f.created_at, f.updated_at,
    f.papel_calc,
...
CREATE OR REPLACE FUNCTION public.rpc_minhas_tarefas_stats(p_incluir_criador boolean DEFAULT true)
RETURNS TABLE(total bigint, ativas bigint, concluidas bigint, concluidas_30d bigint, concluidas_hoje bigint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
  WITH minhas AS (
  ...
  SELECT
    count(*)::bigint,
    count(*) FILTER (WHERE status IS DISTINCT FROM 'concluida')::bigint,
    count(*) FILTER (WHERE status = 'concluida')::bigint,
    count(*) FILTER (WHERE status = 'concluida' AND data_conclusao >= ((now() AT TIME ZONE 'America/Sao_Paulo')::date - 30))::bigint,
    count(*) FILTER (WHERE status = 'concluida' AND data_conclusao = (now() AT TIME ZONE 'America/Sao_Paulo')::date)::bigint
  FROM minhas;
$function$;