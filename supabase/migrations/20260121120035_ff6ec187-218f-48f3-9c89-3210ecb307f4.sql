-- =====================================================
-- 1. CORRIGIR STATUS DAS CONTAS EXISTENTES
-- =====================================================

-- Atualizar contas que têm data_pagamento mas status != 'pago'
UPDATE contas_pagar
SET 
  status = 'pago',
  updated_at = now()
...
-- Atualizar contas vencidas (sem pagamento e vencimento < hoje)
UPDATE contas_pagar
SET 
  status = 'vencido',
  updated_at = now()
...
-- Atualizar contas pendentes (sem pagamento e vencimento >= hoje)
UPDATE contas_pagar
SET 
  status = 'pendente',
  updated_at = now()
...
-- Tratar parcialmente pagas (valor_pago > 0 mas valor_aberto > 0)
UPDATE contas_pagar
SET 
  status = 'parcial',
  updated_at = now()
...
-- =====================================================
-- 2. ATUALIZAR FUNÇÃO bulk_upsert_contas_pagar_v2 COM MELHOR LÓGICA DE STATUS
-- =====================================================
...
BEGIN
  ...
  WITH inserted AS (
    ...
    SELECT 
      ...
      CASE 
        ...
        WHEN s.valor_aberto = 0 AND s.valor_pago > 0 THEN 'pago'
  ...
  -- UPDATE registros que mudaram (hash diferente) OU que têm status incorreto
  WITH updated AS (
