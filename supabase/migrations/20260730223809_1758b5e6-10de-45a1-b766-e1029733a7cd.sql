...
CREATE OR REPLACE FUNCTION public.china_set_doc_tarefa(p_doc_id uuid, p_tarefa_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.china_produto_documentos d
    WHERE d.projeto_tarefa_id = p_tarefa_id AND d.id <> p_doc_id
  ) THEN
    UPDATE public.china_produto_documentos SET projeto_tarefa_id = NULL
     WHERE id = p_doc_id AND projeto_tarefa_id IS DISTINCT FROM NULL;
  ELSE
    UPDATE public.china_produto_documentos SET projeto_tarefa_id = p_tarefa_id
     WHERE id = p_doc_id AND projeto_tarefa_id IS DISTINCT FROM p_tarefa_id;
  END IF;
END;
$function$;
...
BEGIN
  ...
  WITH chaves AS (
    SELECT v.tarefa_id, min(d.tipo_documento) AS tipo_key
    FROM public.china_documento_tarefa_vinculos v
    JOIN public.china_produto_documentos d ON d.id = v.documento_id
    WHERE v.projeto_id = v_projeto_id AND d.submissao_id = p_submissao_id
  ...
  LOOP
    ...
    PERFORM public.china_set_doc_tarefa(v_doc.id, v_alvo);
  ...
  LOOP
    ...
    FOR v_mig IN
      SELECT d.id FROM public.china_produto_documentos d
      WHERE d.projeto_tarefa_id = ANY(v_dup.todas) AND d.projeto_tarefa_id <> v_dup.manter
      ORDER BY d.created_at
    LOOP
      PERFORM public.china_set_doc_tarefa(v_mig.id, v_dup.manter);
    END LOOP;
  ...
  IF EXISTS (
    ...
    WHERE t.projeto_id = v_projeto_id AND t.tipo_tarefa = 'china_documento'
      AND t.excluida_em IS NULL AND t.deleted_at IS NULL
      AND COALESCE(t.campos_customizados->>'china_tipo_key','') ~ '^cofre_'
  ) THEN
    SELECT id INTO v_secao_cofre FROM public.projeto_secoes
    WHERE projeto_id = v_projeto_id AND nome = 'Cofre do Produto' LIMIT 1;

    IF v_secao_cofre IS NULL THEN
      ...
      VALUES (v_projeto_id, 'Cofre do Produto', COALESCE(v_secao_ordem, 1))
      RETURNING id INTO v_secao_cofre;
    END IF;

    UPDATE public.projeto_tarefas t
       SET secao_id = v_secao_cofre
     WHERE t.projeto_id = v_projeto_id AND t.tipo_tarefa = 'china_documento'
       AND t.excluida_em IS NULL AND t.deleted_at IS NULL
       AND COALESCE(t.campos_customizados->>'china_tipo_key','') ~ '^cofre_'
       AND t.secao_id IS DISTINCT FROM v_secao_cofre;

    UPDATE public.china_documento_tarefa_vinculos v
       SET secao_id = v_secao_cofre
      FROM public.projeto_tarefas t
     WHERE v.tarefa_id = t.id AND v.projeto_id = v_projeto_id
       AND t.secao_id = v_secao_cofre AND v.secao_id IS DISTINCT FROM v_secao_cofre;
  END IF;
  ...
  DELETE FROM public.projeto_secoes s
   WHERE s.projeto_id = v_projeto_id
     AND s.nome IN ('Outros documentos', 'Cofre do Produto')
     AND NOT EXISTS (
       SELECT 1 FROM public.projeto_tarefas t
       WHERE t.secao_id = s.id AND t.excluida_em IS NULL AND t.deleted_at IS NULL);
