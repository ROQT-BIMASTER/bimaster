-- ============================================
-- MÓDULO TRADE PERFORMANCE - SISTEMA DE GAMIFICAÇÃO
-- ============================================

-- Tabela de configuração de pontos por ação
CREATE TABLE IF NOT EXISTS public.trade_action_points (
...
-- Tabela de rankings consolidados (atualizada via trigger)
CREATE TABLE IF NOT EXISTS public.user_rankings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  period_type VARCHAR(20) NOT NULL, -- monthly, quarterly, yearly, all_time
  period_key VARCHAR(20) NOT NULL, -- YYYY-MM, YYYY-Q1, YYYY, all
...
CREATE TABLE IF NOT EXISTS public.trade_rewards (
  ...
  description TEXT,
  reward_type VARCHAR(50) NOT NULL, -- fixed_prize, commission, hybrid, badge
  min_points INTEGER,
  max_points INTEGER,
  points_value NUMERIC(10,2), -- valor em R$ por ponto (para comissão)
  fixed_amount NUMERIC(10,2), -- valor fixo (para prêmios)
...
CREATE TABLE IF NOT EXISTS public.user_rewards_received (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.trade_challenges (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
BEGIN
  ...
  -- Calcular multiplicador (pode ser expandido com regras complexas)
  -- Por exemplo: verificar compliance, qualidade, etc.
...
DECLARE
  ...
  v_level_info RECORD;
BEGIN
  -- Calcular total de pontos para o período
...
-- Policies para trade_action_points
CREATE POLICY "Todos podem ver configuração de pontos"
  ON public.trade_action_points FOR SELECT
  USING (is_active = true);

CREATE POLICY "Apenas admins e supervisores gerenciam pontos"
  ON public.trade_action_points FOR ALL
  USING (is_admin_or_supervisor(auth.uid()));

-- Policies para user_points_history
...
CREATE POLICY "Sistema insere pontos automaticamente"
  ON public.user_points_history FOR INSERT
  WITH CHECK (true);

-- Policies para user_rankings
CREATE POLICY "Todos podem ver rankings"
  ON public.user_rankings FOR SELECT
  USING (true);

CREATE POLICY "Sistema atualiza rankings automaticamente"
  ON public.user_rankings FOR ALL
  USING (true)
  WITH CHECK (true);

-- Policies para trade_rewards
CREATE POLICY "Todos podem ver recompensas ativas"
  ON public.trade_rewards FOR SELECT
  USING (is_active = true);

CREATE POLICY "Apenas admins gerenciam recompensas"
  ON public.trade_rewards FOR ALL
  USING (is_admin_or_supervisor(auth.uid()));

-- Policies para user_rewards_received
CREATE POLICY "Usuários veem suas recompensas"
  ON public.user_rewards_received FOR SELECT
  USING (user_id = auth.uid() OR is_admin_or_supervisor(auth.uid()));

CREATE POLICY "Usuários solicitam recompensas"
  ON public.user_rewards_received FOR INSERT
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Admins aprovam recompensas"
  ON public.user_rewards_received FOR UPDATE
  USING (is_admin_or_supervisor(auth.uid()));

-- Policies para trade_challenges
CREATE POLICY "Todos podem ver desafios ativos"
  ON public.trade_challenges FOR SELECT
  USING (is_active = true);

CREATE POLICY "Apenas admins gerenciam desafios"
  ON public.trade_challenges FOR ALL
  USING (is_admin_or_supervisor(auth.uid()));

-- Policies para user_challenge_progress
