...
DECLARE
  ...
  v_filter text := '^(contas|dre|fluxo|conciliac|financ|fatur|pagament|receb|banco|cheque|invoice|payment|nota_fiscal|provis|orcament|account_|chart_|bank_|cash_)';
BEGIN
  FOR r IN
  ...
  RAISE NOTICE 'Phase 2B (financeiro): rewrapped %; pending = %', v_count, v_pending;
  IF v_pending > 0 THEN RAISE EXCEPTION 'Phase 2B (financeiro): % pendentes', v_pending; END IF;
END $migration$;