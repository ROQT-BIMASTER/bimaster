...
BEGIN
  ...
  -- Caminhos folha->ancestral com fator cumulativo (qtde por unidade da folha)
  path AS (
  ...
  -- Para cada SKU (folha ou pai), escolhe sua folha canônica = caminho de maior fator
  best_leaf AS (
  ...
  -- Distribui saldo de cada SKU para sua folha canônica em unidades (sem duplicar)
  contrib AS (
    SELECT s.empresa_par,
           bl.cod_folha AS cod_produto,
           SUM(CASE WHEN s.cod_produto = bl.cod_folha THEN s.saldo ELSE 0 END) AS saldo_proprio,
           SUM(CASE WHEN s.cod_produto <> bl.cod_folha THEN s.saldo * bl.fator ELSE 0 END) AS saldo_potencial
    FROM saldos s
    JOIN best_leaf bl ON bl.cod_produto = s.cod_produto
    GROUP BY 1, 2
  ),
  -- Metadados da folha (linha/curva/nome/custo/preço/pedido) vindos do registro de estoque da própria folha
  meta AS (
