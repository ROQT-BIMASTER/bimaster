...
BEGIN
  ...
  v_is_admin := has_role(v_user,'admin'::app_role) OR has_role(v_user,'gerente'::app_role);
  IF v_is_admin THEN
    SELECT array_agg(DISTINCT e.empresa_par) INTO v_empresas FROM erp_estoque_distribuidora e WHERE e.empresa_par IS NOT NULL;
  ELSE
    SELECT array_agg(DISTINCT ue.empresa_id) INTO v_empresas FROM user_empresas ue WHERE ue.user_id = v_user;
  END IF;
  IF v_empresas IS NULL THEN v_empresas := ARRAY[]::integer[]; END IF;
  IF p_empresas IS NOT NULL AND array_length(p_empresas,1)>0 THEN
    v_empresas := ARRAY(SELECT unnest(v_empresas) INTERSECT SELECT unnest(p_empresas));
  ...
  base AS (
    SELECT COALESCE(sf.cod_produto, po.cod_produto) AS cod_produto,
  ...
  agg AS (
    SELECT
      f.cod_produto,
      SUM(f.saldo_proprio) AS saldo_proprio,
      SUM(f.saldo_potencial) AS saldo_potencial,
      SUM(f.pedido_pendente) AS pedido_pendente
    FROM f
    GROUP BY f.cod_produto
  ),
  flt AS (
    SELECT * FROM agg a
    WHERE NOT p_apenas_com_saldo
       OR (CASE WHEN p_incluir_potencial THEN a.saldo_proprio + a.saldo_potencial ELSE a.saldo_proprio END) > 0
  )
  SELECT
    COUNT(*)::bigint,
    SUM(CASE WHEN p_incluir_potencial THEN flt.saldo_proprio + flt.saldo_potencial ELSE flt.saldo_proprio END)::numeric,
    SUM(flt.saldo_potencial)::numeric,
    SUM(flt.pedido_pendente)::numeric,
    COUNT(*) FILTER (WHERE (CASE WHEN p_incluir_potencial THEN flt.saldo_proprio + flt.saldo_potencial ELSE flt.saldo_proprio END) <= 0)::bigint
  FROM flt;
END $$;

GRANT EXECUTE ON FUNCTION public.rpc_estoque_cores_kpis_consolidado(
