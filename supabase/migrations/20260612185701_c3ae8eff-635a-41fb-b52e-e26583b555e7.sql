DROP FUNCTION IF EXISTS public.rpc_estoque_cores_kpis(integer[], text[], uuid[], text, boolean, boolean, text[], text[], boolean);

CREATE OR REPLACE FUNCTION public.rpc_estoque_cores_kpis(p_empresas integer[] DEFAULT NULL::integer[], p_linhas text[] DEFAULT NULL::text[], p_campanha_ids uuid[] DEFAULT NULL::uuid[], p_busca text DEFAULT NULL::text, p_apenas_com_saldo boolean DEFAULT false, p_com_pedido_pendente boolean DEFAULT false, p_curva_fisica text[] DEFAULT NULL::text[], p_curva_monetaria text[] DEFAULT NULL::text[], p_incluir_potencial boolean DEFAULT true)
 RETURNS TABLE(total_skus bigint, total_unidades numeric, total_unidades_potencial numeric, total_custo numeric, total_valor_venda numeric, total_pedido_pendente numeric, itens_sem_saldo bigint, total_bloqueado_produto numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  saldos AS (
    SELECT e.empresa_par, e.cod_produto,
           COALESCE(e.saldo,0)::numeric AS saldo,
           COALESCE(e.estoque_bloqueado_produto,0)::numeric AS bloqueado
  ...
  contrib AS (
    SELECT m.empresa_par,
           ...
           SUM(CASE WHEN m.cod_sku = m.cod_folha THEN m.bloqueado ELSE 0 END) AS bloqueado_proprio,
           SUM(CASE WHEN m.cod_sku <> m.cod_folha THEN m.bloqueado * m.fator ELSE 0 END) AS bloqueado_potencial
    FROM mapa m
    GROUP BY 1, 2
  ),
  meta AS (
  ...
  base AS (
    SELECT c.empresa_par, c.cod_produto,
           ...
           c.bloqueado_proprio, c.bloqueado_potencial,
  ...
  SELECT
    ...
    SUM(CASE WHEN p_incluir_potencial THEN f.bloqueado_proprio + f.bloqueado_potencial ELSE f.bloqueado_proprio END)::numeric
  FROM f;
END
$function$;