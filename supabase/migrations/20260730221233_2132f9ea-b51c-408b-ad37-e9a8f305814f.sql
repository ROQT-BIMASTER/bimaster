CREATE OR REPLACE FUNCTION public.rpc_china_criar_projeto_espelho(
  p_submissao_id uuid, p_projeto_id uuid DEFAULT NULL::uuid, p_template_b2c_id uuid DEFAULT NULL::uuid,
  p_secao_nome text DEFAULT 'Documentos da Submissão'::text, p_projeto_nome text DEFAULT NULL::text,
  p_data_inicio date DEFAULT NULL::date, p_data_fim_alvo date DEFAULT NULL::date,
  p_prazo_padrao_tarefa integer DEFAULT NULL::integer, p_alerta_antecipacao_dias integer DEFAULT NULL::integer,
  p_regime_calendario text DEFAULT NULL::text, p_usa_feriados boolean DEFAULT NULL::boolean,
  p_uf_feriados text DEFAULT NULL::text, p_substituir boolean DEFAULT false,
...
BEGIN
  ...
  IF v_already_espelho IS NOT NULL THEN
    IF p_substituir THEN
      ...
      WHERE projeto_id = v_already_espelho
        AND documento_id IN (
          SELECT id FROM public.china_produto_documentos WHERE submissao_id = p_submissao_id);

      UPDATE public.china_produto_documentos
        SET projeto_tarefa_id = NULL WHERE submissao_id = p_submissao_id;
    ...
    ELSE
      RETURN jsonb_build_object(
        'projeto_id', v_already_espelho, 'submissao_id', p_submissao_id,
        'created', false, 'already_existed', true);
    END IF;
  END IF;
  ...
  IF p_projeto_id IS NOT NULL THEN
    ...
    UPDATE public.projetos
      SET data_inicio = COALESCE(p_data_inicio, data_inicio),
  ...
  ELSE
    INSERT INTO public.projetos (
      ...
      '#E91E78', 'Package', 'ativo', 'privado', 'china_submissao', v_user, 'china_submissao',
      p_data_inicio, p_data_fim_alvo,
      COALESCE(p_prazo_padrao_tarefa, 5), COALESCE(p_alerta_antecipacao_dias, 2),
      COALESCE(p_regime_calendario, 'dias_uteis'), COALESCE(p_usa_feriados, true),
  ...
  ON CONFLICT (submissao_id) DO UPDATE
    SET projeto_id = EXCLUDED.projeto_id, is_espelho = true;

  -- ============ MODO NOVO: estrutura (categoria -> tipos) ============
  IF v_usou_estrutura THEN
    FOR v_cat IN SELECT * FROM jsonb_array_elements(p_estrutura)
    LOOP
      v_cat_key   := COALESCE(v_cat->>'categoria_key', '');
      v_cat_label := COALESCE(NULLIF(trim(v_cat->>'categoria_label'), ''), v_cat_key, 'Categoria');
      v_secao_ordem := v_secao_ordem + 1;

      SELECT id INTO v_secao_id FROM public.projeto_secoes
      WHERE projeto_id = v_projeto_id AND nome = v_cat_label LIMIT 1;
      ...
      IF jsonb_typeof(v_cat->'tipos') = 'array' THEN
        ...
        LOOP
          ...
          v_tipo_label := COALESCE(NULLIF(trim(v_tipo->>'label'), ''), v_tipo_key, 'Item do checklist');
          IF v_tipo_key = '' THEN CONTINUE; END IF;

          v_tarefa_ordem := v_tarefa_ordem + 1;

          SELECT id INTO v_tarefa_id FROM public.projeto_tarefas
          WHERE projeto_id = v_projeto_id AND secao_id = v_secao_id AND titulo = v_tipo_label
          LIMIT 1;
          ...
          IF v_tarefa_id IS NULL THEN
            INSERT INTO public.projeto_tarefas (
              ...
              jsonb_build_object('china_tipo_key', v_tipo_key,
                                 'china_categoria_key', v_cat_key,
                                 'china_submissao_id', p_submissao_id))
            RETURNING id INTO v_tarefa_id;
          ELSE
            UPDATE public.projeto_tarefas
              SET campos_customizados = COALESCE(campos_customizados, '{}'::jsonb)
                || jsonb_build_object('china_tipo_key', v_tipo_key,
                                      'china_categoria_key', v_cat_key,
                                      'china_submissao_id', p_submissao_id)
  ...
  ELSE
    -- ============ MODO LEGADO ============
    SELECT id INTO v_secao_id FROM public.projeto_secoes
    WHERE projeto_id = v_projeto_id AND nome = p_secao_nome LIMIT 1;
    ...
    SELECT * INTO v_checklist FROM public.china_produto_checklist
    WHERE submissao_id = p_submissao_id LIMIT 1;
    ...
    IF v_checklist.id IS NOT NULL AND jsonb_typeof(v_checklist.colunas) = 'array' THEN
      ...
      LOOP
        ...
        INSERT INTO public.projeto_tarefas (
          ...
          (COALESCE(p_data_inicio, current_date) + v_prazo_padrao)::date,
          jsonb_build_object('china_tipo_key', v_col_key, 'china_submissao_id', p_submissao_id))
        RETURNING id INTO v_tarefa_id;
      END LOOP;
    END IF;
  END IF;

  -- ============ Documentos: rotina única de sincronização ============
  PERFORM public.rpc_china_sincronizar_documentos_projeto(p_submissao_id, NULL, v_user);

  -- Template Brasil->China opcional
  IF p_template_b2c_id IS NOT NULL THEN
    FOR v_item IN
      SELECT jsonb_array_elements(itens)
      FROM public.china_checklist_brasil_china_templates WHERE id = p_template_b2c_id
    LOOP
  ...
  BEGIN
    INSERT INTO public.china_timeline_eventos (
      ...
      'projeto_espelho_criado', 'Projeto-espelho criado',
      ...
      jsonb_build_object('projeto_id', v_projeto_id, 'template_b2c_id', p_template_b2c_id,
                         'substituiu', p_substituir, 'modo_estrutura', v_usou_estrutura),
      'brasil');
  EXCEPTION WHEN OTHERS THEN NULL;
  END;

  RETURN jsonb_build_object(
    'projeto_id', v_projeto_id, 'submissao_id', p_submissao_id, 'secao_id', v_secao_id,
    'created', true, 'already_existed', false, 'modo_estrutura', v_usou_estrutura);
END;
$function$;