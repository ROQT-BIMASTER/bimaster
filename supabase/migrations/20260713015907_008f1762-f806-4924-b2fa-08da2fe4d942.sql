...
BEGIN
  ...
  -- Se essa invariante for violada após o refresh, é regressão
  -- da regra e o WARNING abaixo dispara.
  -- ============================================================
  ...
  folhas AS (
    SELECT s.cod_produto AS cod FROM todos_skus s
    LEFT JOIN paes p ON p.cod = s.cod_produto WHERE p.cod IS NULL
  ),
  -- Recursivo apenas para descobrir a raiz mais provável de cada nó
  -- (fator/quantidade continuam sendo calculados por refresh_estoque_unificado_cache).
  rec AS (
  ...
  SELECT
    ...
    CASE
      ...
      WHEN s.cod_produto IN (SELECT cod FROM paes)                THEN 2  -- é pai (e portanto também aparece como filho)
      ELSE                                                              3  -- não é pai → folha (inclui SKUs fora da BOM)
    END AS nivel,
  ...
  -- Guard permanente contra regressão: nenhum nível 3 pode ter fator > 1.
