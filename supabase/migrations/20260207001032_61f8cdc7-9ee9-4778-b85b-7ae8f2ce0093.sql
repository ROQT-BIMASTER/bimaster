...
-- =============================================
-- Inteligência Municipal - RPCs para análise de 5.571 municípios
-- =============================================

-- 1. Função principal: dados paginados com cruzamento comercial
...
RETURNS TABLE (
  ...
  clientes_com_compra bigint,
  receita_total numeric,
  receita_maior numeric,
  ticket_medio numeric,
  total_prospects bigint,
  total_leads bigint,
  densidade_comercial numeric,
  intensidade_comercial numeric,
...
BEGIN
  ...
  prosp_agg AS (
    ...
    FROM ibge_municipios im2
    INNER JOIN prospects p ON LOWER(TRIM(unaccent(p.nome_empresa))) IS NOT NULL
      AND p.municipio_id IS NOT NULL
    -- Use municipio_id from prospects table directly mapped via municipios
    GROUP BY im2.id
  ),
  -- Simple prospect count by matching prospect's municipio -> municipios -> ibge
  prosp_simple AS (
    SELECT
      m.id AS mun_id,
      COUNT(p.id)::bigint AS total_prospects
    FROM municipios m
    INNER JOIN prospects p ON p.municipio_id = m.id
    INNER JOIN ibge_municipios im3 ON LOWER(TRIM(unaccent(im3.nome))) = LOWER(TRIM(unaccent(m.nome)))
      AND im3.uf_sigla = m.uf
    GROUP BY m.id
  ),
  -- Alternative: count prospects directly by UF + city name matching
  prosp_by_city AS (
    SELECT
      im4.id AS municipio_id,
      COUNT(DISTINCT p2.id)::bigint AS total_prospects
    FROM ibge_municipios im4
    INNER JOIN municipios m2 ON LOWER(TRIM(unaccent(m2.nome))) = LOWER(TRIM(unaccent(im4.nome)))
      AND m2.uf = im4.uf_sigla
    INNER JOIN prospects p2 ON p2.municipio_id = m2.id
    GROUP BY im4.id
  ),
  leads_agg AS (
    SELECT
    ...
    FROM ibge_municipios im5
    INNER JOIN leads_minerados lm ON LOWER(TRIM(unaccent(lm.cidade))) = LOWER(TRIM(unaccent(im5.nome)))
  ...
  vendedor_agg AS (
    ...
    INNER JOIN profiles pr ON pr.id = vt.vendedor_id
...
-- 2. Função auxiliar: KPIs agregados
...
GRANT EXECUTE ON FUNCTION public.fn_get_municipios_intelligence TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_get_municipios_kpis TO authenticated;
