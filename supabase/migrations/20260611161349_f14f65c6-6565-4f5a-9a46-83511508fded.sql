CREATE OR REPLACE FUNCTION public.rpc_china_criar_projeto_espelho(
  ...
  p_data_fim_alvo date DEFAULT NULL,
  p_prazo_padrao_tarefa int DEFAULT NULL,
  p_alerta_antecipacao_dias int DEFAULT NULL,
  p_regime_calendario text DEFAULT NULL,
  p_usa_feriados boolean DEFAULT NULL,
  p_uf_feriados text DEFAULT NULL,
...
BEGIN
  ...
  IF p_projeto_id IS NOT NULL THEN
    ...
    -- Aplica config (somente quando informada) também em projeto existente
  ...
  LOOP
    ...
    INSERT INTO public.projeto_tarefas (
      ...
      v_ordem,
      (COALESCE(p_data_inicio, current_date) + COALESCE(v_chk.prazo_dias, v_prazo_padrao))::date
    )
...
REVOKE ALL ON FUNCTION public.rpc_china_criar_projeto_espelho(uuid, uuid, uuid, text, text, date, date, int, int, text, boolean, text, boolean) FROM public;
GRANT EXECUTE ON FUNCTION public.rpc_china_criar_projeto_espelho(uuid, uuid, uuid, text, text, date, date, int, int, text, boolean, text, boolean) TO authenticated;