-- ============================================================================
-- FASE 1: CORREÇÕES CRÍTICAS DE SEGURANÇA (v2 - Correção de políticas)
-- ============================================================================

-- 1. CORREÇÃO: Exposição de Dados de PROSPECTS
-- Remove TODAS as políticas existentes da tabela prospects
DO $$ 
DECLARE
    pol_name text;
BEGIN
    FOR pol_name IN 
        SELECT policyname 
        FROM pg_policies 
        WHERE tablename = 'prospects' AND schemaname = 'public'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.prospects', pol_name);
    END LOOP;
END $$;
...
-- Remove TODAS as políticas existentes da tabela atividades
DO $$ 
DECLARE
    pol_name text;
BEGIN
    FOR pol_name IN 
        SELECT policyname 
        FROM pg_policies 
        WHERE tablename = 'atividades' AND schemaname = 'public'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.atividades', pol_name);
    END LOOP;
END $$;
...
-- Remove TODAS as políticas existentes da tabela ai_insights
DO $$ 
DECLARE
    pol_name text;
BEGIN
    FOR pol_name IN 
        SELECT policyname 
        FROM pg_policies 
        WHERE tablename = 'ai_insights' AND schemaname = 'public'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.ai_insights', pol_name);
    END LOOP;
END $$;
