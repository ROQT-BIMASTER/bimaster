...
BEGIN
  ...
  prosp_agg AS (
    ...
    INNER JOIN public.prospects p ON LOWER(TRIM(public.unaccent(p.nome_empresa))) IS NOT NULL
  ...
  prosp_simple AS (
    ...
    INNER JOIN public.ibge_municipios im3 ON LOWER(TRIM(public.unaccent(im3.nome))) = LOWER(TRIM(public.unaccent(m.nome)))
  ...
  vendedor_erp AS (
    SELECT DISTINCT ON (x.ibge_municipio_id)
      x.ibge_municipio_id,
      x.vendedor,
      x.qtd,
      x.total_vendedores
    FROM (
      SELECT
        ...
        COUNT(*) AS qtd,
        COUNT(*) OVER (PARTITION BY c.ibge_municipio_id) AS total_vendedores
      FROM public.clientes c
      WHERE c.ibge_municipio_id IS NOT NULL
        AND LENGTH(TRIM(COALESCE(c.cnpj, ''))) = 14
    ...
    ORDER BY x.ibge_municipio_id, x.qtd DESC, x.vendedor
  ),
  vendedor_agg AS (
  ...
  base AS (
    SELECT
      ...
      COALESCE(
        va.vendedor_nome,
        ve.vendedor || CASE WHEN ve.total_vendedores > 1
                            THEN ' (+' || (ve.total_vendedores - 1) || ')'
                            ELSE '' END
      ) AS vendedor_nome
    ...
    LEFT JOIN vendedor_erp ve ON ve.ibge_municipio_id = im.id
    WHERE 1=1
