-- Phase 2B (final): catch-all para policies pendentes em qualquer tabela public.*
DO $migration$
DECLARE
...
BEGIN
  ...
  WHERE schemaname='public'
    AND (regexp_replace(COALESCE(qual,''),'\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)'
      OR regexp_replace(COALESCE(with_check,''),'\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)');
  RAISE NOTICE 'Phase 2B (final): rewrapped %; pending global = %', v_count, v_pending;
  IF v_pending > 0 THEN RAISE EXCEPTION 'Phase 2B (final): % policies ainda pendentes', v_pending; END IF;
END $migration$;