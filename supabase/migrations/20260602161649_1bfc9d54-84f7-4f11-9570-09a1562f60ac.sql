...
RETURNS TABLE (
  ...
  skus_ativos bigint,
  skus_sem_saldo bigint,
  skus_negativos bigint,
  pedidos_pendentes_qtd numeric,
  skus_com_pendente bigint,
...
BEGIN
  ...
  SELECT
    b.empresa_par,
    MAX(b.abrev_par) AS abrev_par,
    COALESCE(SUM(b.custo_total), 0) AS valor_total,
    COALESCE(SUM(b.saldo), 0) AS unidades_total,
    COUNT(*) AS total_registros,
    COUNT(*) FILTER (WHERE b.saldo > 0) AS skus_ativos,
    COUNT(*) FILTER (WHERE b.saldo = 0) AS skus_sem_saldo,
    COUNT(*) FILTER (WHERE b.saldo < 0) AS skus_negativos,
    COALESCE(SUM(b.pedido_pendente), 0) AS pedidos_pendentes_qtd,
    COUNT(*) FILTER (WHERE b.pedido_pendente > 0) AS skus_com_pendente,
    MAX(b.sincronizado_em) AS ultima_sync
  FROM base b
  GROUP BY b.empresa_par
