-- Restaurar bloqueado/disponivel/pendente em UN no cache de Estoque Unificado
-- Mantém a lógica nova de composição via erp_composicao_produto
...
-- View por SKU recriada com bloqueado/pendente/disponivel
...
classificado AS (
  SELECT es.empresa, es.cod_produto, es.nome_prod, es.abrev_par,
         es.saldo, es.bloqueado_total, es.pendente_total,
         GREATEST(es.saldo - es.bloqueado_total, 0) AS disponivel_total,
         es.custo_total,
         n.nivel,
         COALESCE(n.produto_raiz, es.cod_produto) AS produto_raiz
  FROM estoque es
...
SELECT c.empresa,
       ...
       (c.bloqueado_total  * COALESCE(fps.fator_un, 1))::numeric AS bloqueado_em_un,
       (c.disponivel_total * COALESCE(fps.fator_un, 1))::numeric AS disponivel_em_un,
       (c.pendente_total   * COALESCE(fps.fator_un, 1))::numeric AS pendente_em_un
FROM classificado c
...
-- Repopula o cache imediatamente
