-- Phase 2B (1/N): rewrap auth.uid()/jwt()/role() em policies RLS — module: ai
-- pg_policies renderiza a forma já reescrita como "( SELECT auth.uid() AS uid)" — sentinel via regex.
DO $migration$
DECLARE
  r RECORD;
  v_qual text;
  ...
  FUNCTION_PLACEHOLDER text; -- (unused, kept for grep)
BEGIN
  FOR r IN
    SELECT schemaname, tablename, policyname, cmd, permissive, roles, qual, with_check
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename ~* '^(ai_|sofia|chat|llm)'
      AND (
        regexp_replace(COALESCE(qual,''),       '\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)'
        OR
        regexp_replace(COALESCE(with_check,''), '\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)'
      )
  LOOP
    v_qual  := r.qual;
    ...
    IF v_qual IS NOT NULL THEN
      ...
      v_qual := replace(v_qual, '__S_UID__', '(select auth.uid())');
      v_qual := replace(v_qual, '__S_JWT__', '(select auth.jwt())');
      v_qual := replace(v_qual, '__S_ROL__', '(select auth.role())');
    END IF;
    ...
    IF v_check IS NOT NULL THEN
      ...
      v_check := replace(v_check, '__S_UID__', '(select auth.uid())');
      v_check := replace(v_check, '__S_JWT__', '(select auth.jwt())');
      v_check := replace(v_check, '__S_ROL__', '(select auth.role())');
    END IF;
    ...
    EXECUTE format(
      ...
      v_perm, r.cmd, v_roles_csv, v_using, v_with
    );
    v_count := v_count + 1;
  END LOOP;
  ...
  RAISE NOTICE 'Phase 2B (ai): rewrapped % policies; pending after = %', v_count, v_pending;

  IF v_pending > 0 THEN
    RAISE EXCEPTION 'Phase 2B (ai): % policies ainda pendentes — abortar.', v_pending;
  END IF;
END $migration$;