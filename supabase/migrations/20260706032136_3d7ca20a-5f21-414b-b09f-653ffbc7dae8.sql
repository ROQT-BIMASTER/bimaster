...
-- 1) Departamentos (Heatmap + KPIs)
DROP FUNCTION IF EXISTS public.fn_despesas_departamentos(integer, date, integer[], text, numeric, boolean);
...
CREATE OR REPLACE FUNCTION public.fn_despesas_departamentos(
  ...
  p_centro_custo_ids uuid[] DEFAULT NULL
)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  RETURN (
    WITH base_raw AS (
      ...
      WHERE cp.status <> 'cancelado'
        ...
        AND (p_centro_custo_ids IS NULL OR cp.centro_custo_id = ANY(p_centro_custo_ids))
...
-- 2) Drill (plano | fornecedor | titulos)
DROP FUNCTION IF EXISTS public.fn_despesas_drill(text, date, uuid, boolean, uuid, text, integer[], text, integer, integer);
...
-- 3) Variações do mês
DROP FUNCTION IF EXISTS public.fn_despesas_variacoes(date, integer[], text, numeric, integer);
...
-- 4) Centros de custo com lançamento no período
CREATE OR REPLACE FUNCTION public.fn_torre_centros_custo_disponiveis(
  p_meses integer DEFAULT 13,
  p_mes_ref date DEFAULT NULL,
  p_empresa_ids integer[] DEFAULT NULL
)
...
BEGIN
  ...
  v_mes_ref := COALESCE(date_trunc('month', p_mes_ref)::date,
                        date_trunc('month', current_date)::date);
  v_ini     := (v_mes_ref - make_interval(months => v_meses - 1))::date;
  v_fim     := (v_mes_ref + interval '1 month')::date;
  ...
  RETURN COALESCE((
    SELECT jsonb_agg(jsonb_build_object(
           ...
           ) ORDER BY x.valor DESC, cc.nome)
    FROM (
      SELECT cp.centro_custo_id AS id,
             count(*)           AS qtd,
      ...
      WHERE cp.status <> 'cancelado'
        ...
        AND (p_empresa_ids IS NULL OR cp.empresa_id = ANY(p_empresa_ids))
        AND cp.data_emissao IS NOT NULL
        AND cp.data_emissao >= v_ini
        AND cp.data_emissao <  v_fim
      GROUP BY cp.centro_custo_id
    ) x
    JOIN public.centros_custo cc ON cc.id = x.id
  ), '[]'::jsonb);
END;
$function$;
