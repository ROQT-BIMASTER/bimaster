...
BEGIN
  ...
  v_is_design := v_ext IN ('ai','psd');

  IF v_is_design AND v_size > 1073741824 THEN
    v_code := 'design_size_exceeded';
    v_reason := format('Arquivo de design tem %s bytes e excede o limite de 1 GB', v_size);
  ELSIF v_is_video AND v_size > 104857600 THEN
    v_code := 'video_size_exceeded';
    v_reason := format('Arquivo de vídeo tem %s bytes e excede o limite de 100 MB', v_size);
  ELSIF NOT v_is_video AND NOT v_is_design AND v_size > 20971520 THEN
