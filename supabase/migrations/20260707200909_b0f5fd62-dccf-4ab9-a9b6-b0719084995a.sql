...
BEGIN
  IF p_tabela = 'contas_pagar' THEN
    WITH lote AS (
      SELECT id, plano_contas_id FROM public.contas_pagar
  ...
  ELSIF p_tabela = 'contas_receber' THEN
    WITH lote AS (
      SELECT id, plano_conta_id FROM public.contas_receber
       WHERE plano_snapshot_at IS NULL
       LIMIT p_batch
    )
  ...
  ELSIF p_tabela = 'pagamentos_caixa' THEN
    WITH lote AS (
      SELECT id, plano_contas_id FROM public.pagamentos_caixa
       WHERE plano_snapshot_at IS NULL
       LIMIT p_batch
    )
