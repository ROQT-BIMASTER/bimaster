...
BEGIN
  ...
  -- 0) tarefas automáticas legadas sem chave técnica: deduz do documento vinculado
  WITH chaves AS (
  ...
  -- 1) move documentos para a tarefa/seção correta do checklist
  FOR v_doc IN
  ...
  -- 2) consolida tarefas automáticas duplicadas do mesmo tipo
  FOR v_dup IN
  ...
  -- 3) itens do Cofre do Produto ganham quadro próprio
  IF EXISTS (
  ...
  -- 4) renomeia títulos técnicos para o rótulo legível
  UPDATE public.projeto_tarefas t
  ...
  -- 5) arquiva tarefas automáticas que ficaram vazias
  WITH orfas AS (
    SELECT t.id
    FROM public.projeto_tarefas t
    WHERE t.projeto_id = v_projeto_id
  ...
  -- 6) seções automáticas vazias são removidas
