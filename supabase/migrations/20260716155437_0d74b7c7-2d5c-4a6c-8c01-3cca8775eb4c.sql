...
CREATE OR REPLACE FUNCTION public.get_minhas_tarefas_central(
  p_limite_concluidas int DEFAULT NULL,
  p_incluir_criador boolean DEFAULT false
)
...
AS $function$
  WITH minhas AS (
    ...
    SELECT * FROM dedup WHERE status <> 'concluida' OR status IS NULL
  ...
  SELECT
    f.id, f.titulo, f.descricao, f.status, f.prioridade, f.data_inicio_planejada, f.data_prazo, f.data_conclusao,
    f.projeto_id,
    COALESCE(p.nome, 'Sem projeto'),
    COALESCE(p.cor, '#6366f1'),
    f.estagio, f.criador_id, f.visibilidade, f.secao_id,
    s.nome,
    COALESCE(f.ordem, 0),
    f.parent_tarefa_id, f.responsavel_id,
    pr.nome, pr.avatar_url,
    f.codigo, f.produto_id, f.created_at, f.updated_at, f.papel_calc,
...
GRANT EXECUTE ON FUNCTION public.get_minhas_tarefas_central(int, boolean) TO authenticated;

-- Alinhar rpc_minhas_tarefas_stats: por padrão exclui tarefas em que o usuário
-- é apenas criador (sem responsabilidade/colaboração/seguidor). Mantém retro-
-- compatibilidade via parâmetro opcional p_incluir_criador.
...
REVOKE ALL ON FUNCTION public.rpc_minhas_tarefas_stats(boolean) FROM anon;
