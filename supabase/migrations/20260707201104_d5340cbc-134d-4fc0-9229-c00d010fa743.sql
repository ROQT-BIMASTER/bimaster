...
BEGIN
  IF p_tabela = 'contas_pagar' THEN
    WITH lote AS (
      SELECT ctid, plano_contas_id FROM public.contas_pagar
    ...
    UPDATE public.contas_pagar cp
      ...
      FROM lote l
      LEFT JOIN public.trade_chart_of_accounts t ON t.id = l.plano_contas_id
     WHERE cp.ctid = l.ctid;
    GET DIAGNOSTICS v_affected = ROW_COUNT;

  ELSIF p_tabela = 'contas_receber' THEN
    WITH lote AS (
      SELECT ctid, plano_conta_id FROM public.contas_receber
       WHERE plano_snapshot_at IS NULL
       LIMIT p_batch
    )
    UPDATE public.contas_receber cr
       SET plano_conta_id_snapshot    = cr.plano_conta_id,
           ...
           plano_snapshot_at          = now()
      FROM lote l
      LEFT JOIN public.trade_chart_of_accounts t ON t.id = l.plano_conta_id
     WHERE cr.ctid = l.ctid;
    GET DIAGNOSTICS v_affected = ROW_COUNT;

  ELSIF p_tabela = 'pagamentos_caixa' THEN
    WITH lote AS (
      SELECT ctid, plano_contas_id FROM public.pagamentos_caixa
       WHERE plano_snapshot_at IS NULL
       LIMIT p_batch
    )
