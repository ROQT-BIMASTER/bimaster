-- Phase 2B (2/N): module admin_security
DO $migration$
DECLARE
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname='public'
      AND tablename ~* '^(user_|profil|role|audit|access_|mfa|quarant|admin)'
      AND (
  ...
  WHERE schemaname='public' AND tablename ~* '^(user_|profil|role|audit|access_|mfa|quarant|admin)'
    AND (
      regexp_replace(COALESCE(qual,''),       '\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)'
      OR regexp_replace(COALESCE(with_check,''), '\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)'
    );
  RAISE NOTICE 'Phase 2B (admin_security): rewrapped %; pending after = %', v_count, v_pending;
  IF v_pending > 0 THEN RAISE EXCEPTION 'Phase 2B (admin_security): % pendentes', v_pending; END IF;
END $migration$;