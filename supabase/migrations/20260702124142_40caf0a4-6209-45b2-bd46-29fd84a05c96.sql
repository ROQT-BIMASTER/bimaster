-- Flag para bloquear projetos globalmente (Controladoria / RR - Tasks)
ALTER TABLE public.projetos
  ADD COLUMN IF NOT EXISTS bloqueado boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_projetos_bloqueado ON public.projetos(bloqueado);

-- Marca os dois projetos como bloqueados (case-insensitive, tolerante a espaços)
UPDATE public.projetos
   SET bloqueado = true
 WHERE lower(btrim(nome)) IN ('controladoria', 'rr - tasks');

-- RPC principal usada pela listagem de projetos: filtra bloqueados para não-admins
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE ur.user_id = _caller_id AND ur.role = 'admin'::public.app_role
  ) INTO _is_admin;
  ...
  IF _include_all AND _can_view_all THEN
    ...
    WHERE p.deleted_at IS NULL
      AND (NOT p.bloqueado OR _is_admin)
  ...
  WHERE p.deleted_at IS NULL
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE pm.projeto_id = p.id AND pm.user_id = _effective_user_id
      )
      OR EXISTS (
        SELECT 1 FROM public.projeto_departamentos pd
        JOIN public.profiles pr ON pr.departamento_id = pd.departamento_id
        WHERE pd.projeto_id = p.id AND pr.id = _effective_user_id
      )
    )
  ORDER BY p.created_at DESC;
END;
