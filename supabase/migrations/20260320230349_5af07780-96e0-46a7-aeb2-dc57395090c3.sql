...
-- MIGRATION Huggs Finance v3 — PARTE 2/2 (CORRECTED v5 — fully validated against actual schema)
-- contas_pagar: no descricao (use fornecedor_nome), no status col named 'status' but has 'status'
-- pagamentos: valor (not valor_pago), no status, no empresa_id
-- erp_sync_log: has sync_status (not status), has tabela_origem/registro_id/proximo_envio from prev migration
-- contas_bancarias: banco (not nome)
-- fornecedores: cnpj (not cpf_cnpj)

-- D. TRIGGERS
CREATE OR REPLACE FUNCTION fn_set_audit_on_update()
RETURNS TRIGGER AS $$
BEGIN
  NEW.data_alt = CURRENT_DATE;
  NEW.hora_alt = CURRENT_TIME;
  IF TG_TABLE_NAME != 'erp_sync_log' THEN NEW.updated_at = NOW(); END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_car_audit ON contas_receber;
CREATE TRIGGER trg_car_audit BEFORE UPDATE ON contas_receber FOR EACH ROW EXECUTE FUNCTION fn_set_audit_on_update();
DROP TRIGGER IF EXISTS trg_par_rec_audit ON parcelas_receber;
CREATE TRIGGER trg_par_rec_audit BEFORE UPDATE ON parcelas_receber FOR EACH ROW EXECUTE FUNCTION fn_set_audit_on_update();
DROP TRIGGER IF EXISTS trg_rec_audit ON recebimentos;
CREATE TRIGGER trg_rec_audit BEFORE UPDATE ON recebimentos FOR EACH ROW EXECUTE FUNCTION fn_set_audit_on_update();
DROP TRIGGER IF EXISTS trg_lcc_audit ON lancamentos_conta_corrente;
CREATE TRIGGER trg_lcc_audit BEFORE UPDATE ON lancamentos_conta_corrente FOR EACH ROW EXECUTE FUNCTION fn_set_audit_on_update();

CREATE OR REPLACE FUNCTION fn_sync_titulo_receber_status()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_par_rec_sync ON parcelas_receber;
CREATE TRIGGER trg_par_rec_sync AFTER INSERT OR UPDATE OR DELETE ON parcelas_receber FOR EACH ROW EXECUTE FUNCTION fn_sync_titulo_receber_status();

-- E1. fn_criar_titulo_receber
CREATE OR REPLACE FUNCTION fn_criar_titulo_receber(payload JSONB)
RETURNS JSONB AS $$
DECLARE
...
-- E2. fn_registrar_recebimento
CREATE OR REPLACE FUNCTION fn_registrar_recebimento(payload JSONB)
RETURNS JSONB AS $$
...
-- E3. fn_enfileirar_erp (erp_sync_log uses sync_status not status)
CREATE OR REPLACE FUNCTION fn_enfileirar_erp(p_empresa_id UUID, p_tabela VARCHAR, p_registro_id UUID, p_payload JSONB DEFAULT NULL)
RETURNS UUID AS $$
DECLARE v_id UUID;
BEGIN
...
-- E4. fn_resumo_financeiro
CREATE OR REPLACE FUNCTION fn_resumo_financeiro(p_empresa_id UUID, p_mes SMALLINT DEFAULT NULL, p_ano SMALLINT DEFAULT NULL)
RETURNS JSONB AS $$
DECLARE
...
-- E5. fn_pesquisar_titulos (contas_pagar has fornecedor_nome not descricao)
CREATE OR REPLACE FUNCTION fn_pesquisar_titulos(p_empresa_id UUID, p_tipo VARCHAR DEFAULT NULL, p_status VARCHAR DEFAULT NULL, p_data_ini DATE DEFAULT NULL, p_data_fim DATE DEFAULT NULL, p_limite INT DEFAULT 50, p_offset INT DEFAULT 0)
RETURNS JSONB AS $$
...
-- F1. vw_contas_receber_completo
CREATE OR REPLACE VIEW vw_contas_receber_completo AS
SELECT cr.id, cr.empresa_id, cr.numero_documento, cr.descricao,
  ...
  (cr.valor_liquido - cr.valor_recebido) AS valor_saldo, cr.status,
  CASE WHEN cr.status IN ('pendente','parcial') AND cr.data_vencimento < CURRENT_DATE THEN TRUE ELSE FALSE END AS esta_vencido,
  cr.num_parcelas,
  (SELECT COUNT(*) FROM parcelas_receber p WHERE p.conta_receber_id=cr.id AND p.status='recebido') AS parcelas_recebidas,
  (SELECT COUNT(*) FROM parcelas_receber p WHERE p.conta_receber_id=cr.id AND p.status='pendente') AS parcelas_pendentes,
  cr.codigo_integracao, cr.enviado_erp, cr.bloqueado, cr.inativo,
  cr.data_inc, cr.hora_inc, cr.user_inc, cr.data_alt, cr.hora_alt, cr.user_alt, cr.created_at, cr.updated_at
FROM contas_receber cr LEFT JOIN fornecedores f ON f.id = cr.cliente_id;
GRANT SELECT ON vw_contas_receber_completo TO anon, authenticated;

-- F2. vw_extrato_conta_corrente (pagamentos.valor, cb.banco, cp.fornecedor_nome)
CREATE OR REPLACE VIEW vw_extrato_conta_corrente AS
  SELECT cb.id AS conta_bancaria_id, cb.banco AS conta_nome,
    ...
    pag.valor * -1 AS valor, cp.fornecedor_nome AS descricao, cp.numero_documento,
    'pagamento'::TEXT AS origem, pag.id AS referencia_id
  FROM pagamentos pag
  ...
  UNION ALL
  SELECT cb.id, cb.banco, 'credito'::TEXT, rec.data_recebimento, rec.valor_recebido,
    cr.descricao, cr.numero_documento, 'recebimento'::TEXT, rec.id
  ...
  UNION ALL
  SELECT lcc.conta_bancaria_id, cb.banco, lcc.tipo,
    lcc.data_lancamento,
    CASE WHEN lcc.tipo='debito' THEN lcc.valor * -1 ELSE lcc.valor END,
    lcc.descricao, lcc.numero_documento, lcc.origem, lcc.id
  FROM lancamentos_conta_corrente lcc
  JOIN contas_bancarias cb ON cb.id = lcc.conta_bancaria_id
  WHERE lcc.inativo = FALSE;
GRANT SELECT ON vw_extrato_conta_corrente TO anon, authenticated;

-- G. GRANTS
GRANT EXECUTE ON FUNCTION fn_criar_titulo_receber(JSONB) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION fn_registrar_recebimento(JSONB) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION fn_enfileirar_erp(UUID,VARCHAR,UUID,JSONB) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION fn_resumo_financeiro(UUID,SMALLINT,SMALLINT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION fn_pesquisar_titulos(UUID,VARCHAR,VARCHAR,DATE,DATE,INT,INT) TO anon, authenticated;
