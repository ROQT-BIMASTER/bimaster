...
CREATE TABLE IF NOT EXISTS public.erp_plano_contas_rubysp (
  rubysp_hist_id   bigint PRIMARY KEY,
  empresa_par      integer,
  descricao        text,
  tipo             integer,
  cod_contabil     text,
  complemento      boolean,
  ccusto_padrao    integer,
  raw              jsonb,
  sincronizado_em  timestamptz,
  created_at       timestamptz NOT NULL DEFAULT now()
);

GRANT SELECT ON public.erp_plano_contas_rubysp TO authenticated;
GRANT ALL    ON public.erp_plano_contas_rubysp TO service_role;

ALTER TABLE public.erp_plano_contas_rubysp ENABLE ROW LEVEL SECURITY;
...
BEGIN
  ...
  SELECT DISTINCT ON (s.rubysp_hist_id)
    ...
    s.descricao,
    CASE WHEN s.tipo = 1 THEN 'RECEITA' ELSE 'DESPESA' END,
