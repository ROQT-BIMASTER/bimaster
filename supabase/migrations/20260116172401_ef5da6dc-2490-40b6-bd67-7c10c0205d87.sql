...
-- Atualizar função de KPIs para aceitar filtro de data específica de vencimento
CREATE OR REPLACE FUNCTION get_contas_receber_dashboard_kpis(
  p_empresas int[] DEFAULT NULL,
...
DECLARE
  ...
  v_data_fim date;
BEGIN
  -- Se filtro de data específica, usar apenas esse dia
  ...
  WITH base AS (
    ...
    WHERE data_vencimento >= v_data_inicio
      ...
      -- Filtro adicional por data de recebimento se especificado
