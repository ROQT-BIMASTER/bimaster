CREATE OR REPLACE FUNCTION public.get_accessible_projetos(
...
BEGIN
  ...
  SELECT p.*
  FROM public.projetos p
  WHERE p.criador_id = _effective_user_id
     OR EXISTS (
       SELECT 1
...
REVOKE ALL ON FUNCTION public.get_accessible_projetos(uuid, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_accessible_projetos(uuid, boolean) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_accessible_projetos(uuid, boolean) TO authenticated;