-- Atualizar função bulk_upsert_contas_pagar_v2 com suporte a force_update
...
BEGIN
  ...
  -- Criar tabela temporária com os registros
  ...
  -- Inserir novos registros (que não existem por erp_id)
  WITH new_records AS (
    ...
    LEFT JOIN contas_pagar cp ON cp.erp_id = t.erp_id
    WHERE cp.id IS NULL
    RETURNING 1
  )
  SELECT count(*) INTO v_inserted FROM new_records;
  
  -- Atualizar registros existentes (com hash diferente OU force_update = true)
  WITH updated_records AS (
    ...
    WHERE cp.erp_id = t.erp_id
      AND (
        p_force_update = true  -- FORÇA update se flag ativa
  ...
  -- Calcular skipped (total - inserted - updated)
