...
DECLARE
  ...
  v_filter text := '^(fabrica|produto|bom|ficha|cost|inci|inventor|mrp|estoque|fornec|composicao|formul|prod_)';
BEGIN
  FOR r IN
  ...
  WHERE schemaname='public' AND tablename ~* v_filter
    AND (regexp_replace(COALESCE(qual,''),'\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)'
      OR regexp_replace(COALESCE(with_check,''),'\(\s*SELECT\s+auth\.(uid|jwt|role)\(\)[^)]*\)','','gi') ~* 'auth\.(uid|jwt|role)\(\)');
  RAISE NOTICE 'Phase 2B (fabrica): rewrapped %; pending = %', v_count, v_pending;
  IF v_pending > 0 THEN RAISE EXCEPTION 'Phase 2B (fabrica): % pendentes', v_pending; END IF;
END $migration$;