...
-- 1) Adiciona coluna metadata em fluxo_aprovacao_instancias (idempotente)
ALTER TABLE public.fluxo_aprovacao_instancias
  ADD COLUMN IF NOT EXISTS metadata jsonb NOT NULL DEFAULT '{}'::jsonb;

-- 2) RPC: cria lote de aprovação interna para itens B2C (Brasil → China)
...
BEGIN
  ...
  END IF;
  IF p_b2c_item_ids IS NULL OR array_length(p_b2c_item_ids, 1) IS NULL THEN
  ...
  -- valida que todos os itens pertencem à submissão
  SELECT count(*) INTO v_valid_count
  FROM public.china_checklist_brasil_china
  WHERE id = ANY(p_b2c_item_ids) AND submissao_id = p_submissao_id;
  IF v_valid_count <> array_length(p_b2c_item_ids, 1) THEN
    RAISE EXCEPTION 'Itens fora desta submissão';
  END IF;
  ...
  VALUES (p_config_id, p_submissao_id, v_projeto_id, p_lote_nome, p_prazo_lote,
          ...
          jsonb_build_object('kind', 'b2c', 'b2c_item_ids', to_jsonb(p_b2c_item_ids)))
  RETURNING id INTO v_inst_id;

  -- marca os itens como em aprovação interna
  UPDATE public.china_checklist_brasil_china
     SET status = 'em_preparacao',
         updated_at = now()
   WHERE id = ANY(p_b2c_item_ids)
     AND status IN ('pendente', 'em_preparacao', 'devolvido_china');
...
REVOKE EXECUTE ON FUNCTION public.rpc_criar_lote_aprovacao_b2c(uuid, uuid, text, uuid[], date, text) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.rpc_criar_lote_aprovacao_b2c(uuid, uuid, text, uuid[], date, text) TO authenticated;

-- 3) Atualiza trigger de conclusão de lote para tratar lotes B2C sem mexer no status da submissão
...
BEGIN
  ...
  -- Lote B2C: aprova/devolve itens do checklist Brasil → China, não a submissão
  IF v_kind = 'b2c' THEN
    SELECT ARRAY(SELECT (jsonb_array_elements_text(NEW.metadata->'b2c_item_ids'))::uuid)
      INTO v_b2c_ids;

    IF NEW.status = 'aprovado' THEN
      UPDATE public.china_checklist_brasil_china
         SET status = 'em_preparacao', updated_at = now()
       WHERE id = ANY(v_b2c_ids);
      ...
      VALUES ('aprovacao_b2c_concluida',
              ...
              jsonb_build_object('instancia_id', NEW.id, 'b2c_item_ids', NEW.metadata->'b2c_item_ids'),
              'aprov_b2c_done_' || NEW.id::text);

    ELSIF NEW.status = 'rejeitado' THEN
      UPDATE public.china_checklist_brasil_china
         SET status = 'pendente', updated_at = now(),
             motivo_devolucao = COALESCE(NEW.lote_nome, NEW.titulo, 'Reprovado internamente')
       WHERE id = ANY(v_b2c_ids);
  ...
  -- Comportamento original (lote C2B / documentos China)
  IF NEW.status = 'aprovado' THEN
