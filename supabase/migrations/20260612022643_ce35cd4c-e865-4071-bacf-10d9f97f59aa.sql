...
-- =====================================================================
-- ESTOQUE CORES SEM DUPLICAÇÃO
-- Rateia o saldo de cada ancestral entre suas folhas-descendentes,
-- garantindo: Σ (saldo_proprio + potencial_rateado) por folha
--           = Σ saldo(node) * fator_un(node)
--           = saldo_total_em_unidades do cache unificado.
-- =====================================================================
...
BEGIN
  ...
  -- fator_un por SKU (mesma regra do Estoque Unificado: MAX no recursivo)
  fator_desc AS (
    SELECT cod_folha AS sku, 1::numeric AS fator FROM folhas
    UNION ALL
  ...
  -- caminhos (folha -> ancestral) com proteção contra ciclos
  caminhos AS (
    SELECT
    ...
    UNION ALL
    SELECT p.cod_folha, c.produto_compo, p.depth + 1, p.visited || c.produto_compo
    FROM caminhos p
    JOIN erp_composicao_produto c ON c.materia_compo = p.cod_ancestor
    WHERE p.depth < 5 AND NOT (c.produto_compo = ANY(p.visited))
  ),
  -- pares únicos (folha, ancestral) — descartando multiplicidades de caminho
  caminhos_uniq AS (
    SELECT DISTINCT cod_folha, cod_ancestor FROM caminhos
  ),
  -- quantas folhas-descendentes cada ancestral tem (denominador do rateio)
  n_folhas_por_ancestor AS (
    SELECT cod_ancestor, COUNT(DISTINCT cod_folha)::numeric AS n_folhas
    FROM caminhos_uniq GROUP BY cod_ancestor
  ),
  saldo_folha AS (
  ...
  -- potencial rateado: cada ancestral contribui saldo * fator_un / n_folhas para CADA folha-descendente
  potencial AS (
    SELECT ep.empresa_par, cu.cod_folha AS cod_produto,
      SUM(COALESCE(ep.saldo, 0) * COALESCE(fps.fator_un, 1) / nfa.n_folhas) AS saldo_potencial,
      jsonb_agg(
        jsonb_build_object(
          'cod_pai', ep.cod_produto,
          'nome_pai', ep.nome_prod,
          'saldo_pai', ep.saldo,
          'fator', (COALESCE(fps.fator_un, 1) / nfa.n_folhas),
          'contribuicao', COALESCE(ep.saldo, 0) * COALESCE(fps.fator_un, 1) / nfa.n_folhas,
          'n_folhas_concorrentes', nfa.n_folhas
        ) ORDER BY (COALESCE(ep.saldo, 0) * COALESCE(fps.fator_un, 1) / nfa.n_folhas) DESC
      ) FILTER (WHERE COALESCE(ep.saldo,0) > 0) AS detalhe
    FROM caminhos_uniq cu
    JOIN n_folhas_por_ancestor nfa ON nfa.cod_ancestor = cu.cod_ancestor
    LEFT JOIN fator_por_sku fps ON fps.sku = cu.cod_ancestor
    JOIN erp_estoque_distribuidora ep
      ON ep.cod_produto = cu.cod_ancestor AND ep.empresa_par = ANY(v_empresas)
    GROUP BY ep.empresa_par, cu.cod_folha
  ),
  base AS (
  ...
  divergencia AS (
    SELECT b.cod_produto,
           (COUNT(DISTINCT b.nome_linha) FILTER (WHERE b.nome_linha IS NOT NULL)) > 1 AS tem_div,
           array_agg(DISTINCT b.nome_linha) FILTER (WHERE b.nome_linha IS NOT NULL) AS linhas_div
    FROM base b GROUP BY b.cod_produto
  ),
  filtrado AS (
    SELECT b.*, COALESCE(d.tem_div, false) AS tem_divergencia_linha_v,
           d.linhas_div AS linhas_divergentes_v
    FROM base b
    LEFT JOIN divergencia d ON d.cod_produto = b.cod_produto
    WHERE (p_linhas IS NULL OR b.nome_linha = ANY(p_linhas))
      ...
      AND (NOT p_apenas_divergencia_linha OR COALESCE(d.tem_div, false))
  ...
  SELECT c.id, c.empresa_par, c.abrev_par, c.cod_produto, c.cod_fabricante,
    ...
    c.tem_divergencia_linha_v AS tem_divergencia_linha,
    c.linhas_divergentes_v AS linhas_divergentes
  FROM counted c
  ORDER BY
...
-- View de conciliação Cores ↔ Unificado (auditoria)
DROP VIEW IF EXISTS public.vw_conciliacao_cores_unificado;
CREATE VIEW public.vw_conciliacao_cores_unificado
...
fator_desc AS (
  SELECT cod_folha AS sku, 1::numeric AS fator FROM folhas
  UNION ALL
  SELECT c.produto_compo, (GREATEST(COALESCE(c.quantidade_compo, 1), 1) * fd.fator)::numeric
  FROM erp_composicao_produto c
  JOIN fator_desc fd ON fd.sku = c.materia_compo
),
fator_por_sku AS (
...
total_cores AS (
  SELECT SUM(COALESCE(e.saldo,0) * COALESCE(fps.fator_un, 1))::numeric AS total_un
  FROM erp_estoque_distribuidora e
  LEFT JOIN fator_por_sku fps ON fps.sku = e.cod_produto
  WHERE e.cod_produto IS NOT NULL AND e.empresa_par IS NOT NULL
)
SELECT
  (SELECT total_un FROM total_cores) AS total_cores_un,
  (SELECT COALESCE(SUM(saldo_total_em_unidades), 0) FROM estoque_unificado_cache) AS total_unificado_un,
  (SELECT total_un FROM total_cores) - (SELECT COALESCE(SUM(saldo_total_em_unidades),0) FROM estoque_unificado_cache) AS diferenca;

GRANT SELECT ON public.vw_conciliacao_cores_unificado TO authenticated;
GRANT SELECT ON public.vw_conciliacao_cores_unificado TO service_role;
