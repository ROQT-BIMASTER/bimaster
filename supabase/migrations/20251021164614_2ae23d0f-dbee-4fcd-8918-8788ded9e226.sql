-- =====================================================
-- MÓDULO FINANCEIRO TRADE MARKETING - MELHORIAS V2
...
-- 9. FUNÇÃO para atualizar reserved_amount
...
-- 10. RLS POLICIES (remover antigas e criar novas)

-- trade_approval_levels
ALTER TABLE public.trade_approval_levels ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins podem gerenciar níveis de aprovação" ON public.trade_approval_levels;
DROP POLICY IF EXISTS "Usuários autenticados podem ver níveis de aprovação" ON public.trade_approval_levels;

CREATE POLICY "trade_approval_levels_admin_all"
  ON public.trade_approval_levels FOR ALL
  USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "trade_approval_levels_select_all"
...
DROP POLICY IF EXISTS "Admins e supervisores podem gerenciar campanhas" ON public.trade_campaigns;
DROP POLICY IF EXISTS "Usuários podem ver campanhas que criaram" ON public.trade_campaigns;
DROP POLICY IF EXISTS "Usuários podem criar campanhas" ON public.trade_campaigns;

CREATE POLICY "trade_campaigns_admin_supervisor_all"
...
DROP POLICY IF EXISTS "Admins e supervisores podem gerenciar reservas" ON public.trade_budget_reserves;
DROP POLICY IF EXISTS "Usuários podem ver reservas relacionadas às suas campanhas" ON public.trade_budget_reserves;

CREATE POLICY "trade_reserves_admin_supervisor_all"
  ON public.trade_budget_reserves FOR ALL
  USING (is_admin_or_supervisor(auth.uid()));

CREATE POLICY "trade_reserves_user_select"
...
DROP POLICY IF EXISTS "Admins podem gerenciar todas aprovações" ON public.trade_approvals;
DROP POLICY IF EXISTS "Usuários podem ver aprovações relacionadas" ON public.trade_approvals;
DROP POLICY IF EXISTS "Aprovadores podem criar aprovações" ON public.trade_approvals;
DROP POLICY IF EXISTS "Aprovadores podem atualizar suas aprovações" ON public.trade_approvals;

CREATE POLICY "trade_approvals_admin_all"
  ON public.trade_approvals FOR ALL
  USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "trade_approvals_user_select"
  ON public.trade_approvals FOR SELECT
  USING (approver_user_id = auth.uid() OR is_admin_or_supervisor(auth.uid()));

CREATE POLICY "trade_approvals_user_insert"
  ON public.trade_approvals FOR INSERT
  WITH CHECK (approver_user_id = auth.uid());

CREATE POLICY "trade_approvals_user_update"
