// Recebe lote do plano de contas Ruby_SP (dbo.Historico) e faz upsert no staging.
// Auth: Bearer RUBYSP_SYNC_TOKEN ou FUTURA_SYNC_TOKEN.
// Contrato: POST { tipo, snapshot_fim, contas: [...] }
// Quando snapshot_fim === true, dispara fn_transform_plano_contas_rubysp após o upsert.
...
const ContaSchema = z.object({
  ...
  complemento: z.boolean().optional().nullable(),
  ccusto_padrao: z.number().int().optional().nullable(),
}).passthrough();

const BodySchema = z.object({
  tipo: z.string(),
  contas: z.array(ContaSchema).min(0).max(5_000),
...
Deno.serve(secureHandler(
  { auth: "none", rateLimit: 60, rateLimitPrefix: "receber-plano-contas-rubysp", skipWaf: true },
  async (req) => {
    ...
    const { contas, snapshot_fim } = parsed.data;
    ...
    let staged = 0;
    const errors: Array<{ rubysp_hist_id: number; error: string }> = [];

    try {
      const chunkSize = 500;
      for (let i = 0; i < contas.length; i += chunkSize) {
        const chunk = contas.slice(i, i + chunkSize);
        const rows = chunk.map((c) => ({
          rubysp_hist_id: c.rubysp_hist_id,
          empresa_par: c.empresa_par ?? null,
          descricao: c.descricao ?? null,
          tipo: c.tipo ?? null,
          cod_contabil: c.cod_contabil ?? null,
          complemento: c.complemento ?? null,
          ccusto_padrao: c.ccusto_padrao ?? null,
          raw: c as unknown as Record<string, unknown>,
          sincronizado_em: now,
        }));
        const { error: upErr } = await supabase
          .from("erp_plano_contas_rubysp")
          .upsert(rows, { onConflict: "rubysp_hist_id" });
        if (upErr) {
          for (const r of chunk) {
            errors.push({ rubysp_hist_id: r.rubysp_hist_id, error: upErr.message });
          }
          continue;
        }
        staged += rows.length;
      ...
      if (snapshot_fim) {
        ...
        if (tErr) {
          errors.push({ rubysp_hist_id: 0, error: `transform: ${tErr.message}` });
