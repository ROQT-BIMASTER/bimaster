...
function transformErpData(erpRecord: any) {
  return {
    ...
    fornecedor_codigo: erpRecord['Código'],
    fornecedor_nome: erpRecord['Cliente'],
    valor_original: erpRecord['Valor_Trc'] || 0,
    valor_aberto: erpRecord['Valor em Aberto'] || 0,
    valor_pago: erpRecord['Valor Pago'] || 0,
    ...
    data_pagamento: erpRecord['Data Pgto'] ? new Date(erpRecord['Data Pgto']).toISOString().split('T')[0] : null,
  ...
  try {
    ...
    if (path.endsWith('/sync') && req.method === 'POST') {
      ...
      return new Response(JSON.stringify({
        ...
        message: `${skipped} registros ignorados (sem alterações)`
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
