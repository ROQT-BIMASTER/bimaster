...
const PROMPTS: Record<(typeof ALLOWED_ACTIONS)[number], string> = {
...
Deno.serve(
  secureHandler(
    { auth: "jwt", rateLimit: 30, rateLimitPrefix: "agency-strategy-ai" },
    async (req) => {
      ...
      const { action, data } = validateBody(body, StrategySchema);

      const systemPrompt = PROMPTS[action] + JSON.stringify(data);
          ...
          messages: [
            {
              role: "system",
              content:
                "Você é um especialista em marketing digital e estratégia para agências. Sempre responda em português brasileiro.",
            },
