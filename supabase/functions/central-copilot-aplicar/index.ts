...
Deno.serve(secureHandler(
  { auth: "jwt", rateLimit: 20, rateLimitPrefix: "central-copilot-aplicar" },
  async (req, ctx) => {
    ...
    const { data: acao, error: acaoErr } = await admin
      .from("central_copilot_acoes")
      .select("id, thread_id, tipo, payload, status, projeto_id, central_copilot_threads!inner(user_id)")
      .eq("id", acao_id)
      .maybeSingle();
    if (acaoErr || !acao) {
    ...
    const thread = (acao as any).central_copilot_threads;
    ...
    if (!acao.projeto_id) {
      return new Response(JSON.stringify({ error: "Ação sem projeto vinculado." }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
