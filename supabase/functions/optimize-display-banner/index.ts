...
Deno.serve(secureHandler({ auth: "jwt", rateLimit: 10, rateLimitPrefix: "optimize-display-banner" }, async (req) => {
  ...
  try {
    ...
    const prompt = `Adapt this product display image for use as a horizontal web banner (aspect ratio approximately 3:1, like 1200x400px). 
IMPORTANT RULES:
- DO NOT crop or cut any part of the original image. The entire display must be fully visible.
...
- The result must look like a polished product hero banner.`;
    ...
    logger.error("optimize-display-banner error:", e);
    return new Response(
