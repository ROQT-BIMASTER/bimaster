# Prompt Lovable — Suporte Help Desk · FASE 2 (Motor de SLA + notificações)

> **Cole este prompt no Lovable.** Depende das **Fases 0 e 1** aplicadas. Liga o SLA de verdade: cálculo de prazos em **horário comercial** na abertura do chamado, **pausa** quando "aguardando usuário" (retomada automática quando o usuário responde), **primeira resposta** detectada por trigger, e **monitor agendado** que marca `em_risco`/`violado` e **notifica** assignee/líderes via `notifications`.
>
> **Impacto controlado:** recria as RPCs `rpc_suporte_abrir_chamado` e `rpc_suporte_mudar_status` (mesmas assinaturas e retornos — o frontend do PR #5 não muda), adiciona 1 coluna (`sla_pausado_em`) e 1 índice em `suporte_tickets`, e cria 1 trigger em `mensagens` com **early-exit** (só age em conversas `tipo='suporte'`; o chat corporativo e o desk de TI legado passam direto). Nada mais é alterado.
>
> Recomendação: aplicar **depois** do teste funcional da Fase 1 no app (abrir chamado pela tela), para isolar variáveis.

## Contexto para o Lovable
Fase 2 do help desk multi-departamento (`docs/ARQUITETURA-SUPORTE-HELPDESK-MULTIDEPARTAMENTO.md`, §6). Modelo de SLA padrão de mercado: primeira resposta ≠ resolução; medição em horário comercial (`suporte_calendarios`, default SP 08:30–17:30); pausa em "aguardando usuário" estende o prazo pelo tempo comercial pausado; estados `dentro → em_risco (<20% restante) → violado`, mais `pausado`/`cumprido`.

## Instruções
Crie **uma migration** com o SQL abaixo (idempotente). Se a extensão **pg_cron** não estiver disponível, o bloco de agendamento apenas emite um NOTICE — nesse caso, agende a chamada de `public.suporte_sla_monitor()` a cada 5 minutos pelo mecanismo padrão do projeto (cron de edge function) e me avise.

```sql
-- =====================================================================
-- SUPORTE HELP DESK — FASE 2 (motor de SLA + notificações)
-- =====================================================================

-- ---------- 1. Coluna de pausa + índice para o trigger de mensagens ----------
...
-- ---------- 2. Horário comercial: somar horas úteis a um timestamp ----------
...
BEGIN
  ...
  -- Sem calendário configurado: cai para horas corridas (24/7)
  ...
  RETURN v_cursor; -- salvaguarda (calendário sem intervalos válidos)
END;
$$;

-- ---------- 3. Horário comercial: horas úteis entre dois timestamps ----------
...
-- ---------- 4. Resolver a policy de SLA (fila × prioridade, com fallback) ----------
...
-- ---------- 5. (Re)calcular prazos de SLA de um ticket ----------
...
-- ---------- 6. Retomar SLA após pausa (estende prazos pelo tempo comercial pausado) ----------
...
-- ---------- 7. RECRIA rpc_suporte_abrir_chamado (mesma assinatura; agora calcula SLA) ----------
...
BEGIN
  ...
  -- FASE 2: prazos de SLA em horário comercial
...
-- ---------- 8. RECRIA rpc_suporte_mudar_status (mesma assinatura; agora pausa/retoma/cumpre) ----------
...
BEGIN
  ...
  -- FASE 2: saindo de "aguardando usuário" → retoma o relógio (estende prazos pela pausa)
  ...
  -- FASE 2: efeitos de SLA do novo status
  ...
  ELSIF p_status = 'resolvido' THEN
    UPDATE public.suporte_tickets
     ...
     WHERE id = p_ticket_id;
  ELSIF v_status_at = 'resolvido' THEN  -- reabertura
...
-- ---------- 9. Trigger em mensagens: primeira resposta + retomada automática ----------
-- Early-exit: só age quando a conversa é tipo='suporte' (novo modelo).
-- Chat corporativo, grupos e o canal legado do TI (conversa única) passam direto.
...
BEGIN
  ...
  IF NEW.remetente_id <> v_t.requester_id
    ...
    -- resposta de agente → carimba primeira resposta (uma vez)
  ...
  ELSIF NEW.remetente_id = v_t.requester_id AND v_t.status = 'aguardando_usuario' THEN
    -- usuário respondeu → retoma o relógio e volta para atendimento
...
-- ---------- 10. Monitor de SLA (marcar em_risco/violado + notificar) ----------
...
DECLARE
  ...
  v_msg   text;
BEGIN
  -- 1) VIOLADOS (prazo de 1ª resposta pendente vencido, ou prazo de resolução vencido)
  FOR r IN
  ...
  -- 2) EM RISCO (restante em horas comerciais < 20% da meta da policy)
  FOR r IN
    SELECT t.id, t.protocolo, t.titulo, t.fila_id, t.assignee_id, f.nome AS fila_nome,
...
-- ---------- 11. Hardening: funções internas não são chamáveis por clients ----------
...
-- ---------- 12. Agendamento do monitor (pg_cron, a cada 5 min) ----------
DO $$
BEGIN
...
-- ---------- 13. Backfill: prazos para chamados novos (modelo conversa-por-chamado) abertos antes desta fase ----------
DO $$
DECLARE r record;
BEGIN
  FOR r IN
...
```sql
-- (a) funções criadas (esperado: 6 linhas)
SELECT proname FROM pg_proc WHERE proname IN
 ('suporte_add_horas_comerciais','suporte_horas_comerciais_entre','suporte_resolver_sla',
  'suporte_recalcular_sla','suporte_retomar_sla','suporte_sla_monitor')
ORDER BY proname;

-- (b) matemática do calendário: sexta 03/07/2026 16:30 (SP) + 2h úteis
--     esperado: 2026-07-06 09:30 (segunda), fuso -03
SELECT public.suporte_add_horas_comerciais('2026-07-03T16:30:00-03'::timestamptz, 2);

-- (c) horas úteis entre sexta 16:30 e segunda 10:30 — esperado: 3.00
SELECT public.suporte_horas_comerciais_entre('2026-07-03T16:30:00-03','2026-07-06T10:30:00-03');

-- (d) trigger instalado em mensagens
SELECT tgname FROM pg_trigger WHERE tgname = 'trg_suporte_on_mensagem';

-- (e) monitor roda sem erro (esperado: {"violados": N, "em_risco": M})
SELECT public.suporte_sla_monitor();

-- (f) job agendado (se pg_cron disponível)
SELECT jobname, schedule FROM cron.job WHERE jobname = 'suporte-sla-monitor';
```

**Teste funcional (no app, flag ligada):** abrir um chamado → conferir que `prazo_primeira_resposta_em` e `prazo_resolucao_em` ficaram preenchidos; responder como agente → `primeira_resposta_em` carimbado; mudar para "Aguardando usuário" → `sla_status='pausado'`; responder como solicitante → volta para "Em atendimento" com prazos estendidos.

## Depois de aplicar
1. Me mande (a)–(f) + advisor (baseline: 510).
2. Nota: as funções internas (seção 11) são `SECURITY DEFINER` sem grant para clients — se o linter reclamar de "function without search_path", todas já têm `SET search_path = public`.
3. O frontend desta fase (badge de SLA/contagem regressiva no desk e em Meus Chamados) entra como commit no PR do suporte — aviso quando estiver na branch.
