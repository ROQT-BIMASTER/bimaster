...
| -- | ------ | ------ | ------ |
| PR-1 | `chore/audit-governance-and-ci` | Governança (CODEOWNERS, CoC, templates, labels) e CI hardening (CodeQL, dependency-review, PR size) | **Em revisão** |
| PR-2 | `docs/audit-architecture-and-modules` | Refresh de `ARCHITECTURE.md`, `MODULES_OVERVIEW.md`, diagramas e `CODE_HEALTH.md` | Próximo |
| PR-3 | `docs/audit-routes-edge-db` | Inventários de rotas, edge functions e schema; refresh de `INFRASTRUCTURE.md`, `DEPLOYMENT.md`, `SECURITY.md`, `PERFORMANCE.md`, `TESTING.md` | Pendente |
...
- [`GITHUB_BRANCH_PROTECTION.md`](./GITHUB_BRANCH_PROTECTION.md) — regras de proteção da `main`, status checks obrigatórios, settings do repo (PR-1).
- `CODE_HEALTH.md` — saúde do código TS (god-files, `any` count, deps circulares). **PR-2.**
