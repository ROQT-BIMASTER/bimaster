...
export function useProjetoAtividades(filter?: InboxFilter) {
  ...
  const { data: atividades = [], isLoading } = useQuery({
    queryKey: ["projeto-atividades", "v2", user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
      // A RLS de `projeto_atividades` (via user_is_projeto_member) já garante que
      // o usuário só receba linhas de projetos onde ele é criador, membro,
      // admin ou gerente geral. Não filtramos mais no cliente.
