import { useState, useCallback, useMemo } from "react";
import { DashboardFiltersBar } from "@/components/painel-executivo/DashboardFilters";
import { useDashboardKPIs, type DashboardFilters } from "@/hooks/useDashboardKPIs";
import { useRankingSupervisores } from "@/hooks/useRankingSupervisores";
import { useRankingVendedores } from "@/hooks/useRankingVendedores";
import { useReceitaMensal } from "@/hooks/useReceitaMensal";
import { useMetasVendas } from "@/hooks/useMetasVendas";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { SmartValue } from "@/components/ui/smart-value";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";
import { TrendingUp, TrendingDown, Minus, Users, UserCheck, ChevronDown, ChevronRight, BarChart3 } from "lucide-react";
import { AreaChart, Area, ResponsiveContainer, Tooltip } from "recharts";
import { formatCurrency } from "@/lib/formatters";
import { cn } from "@/lib/utils";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { useQuery } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
import { useEmpresaContext } from "@/contexts/EmpresaContext";
import { useOperacaoFilter } from "@/hooks/useConfigOperacoes";
import { AdvancedDataTable, type Column } from "@/components/shared/AdvancedDataTable";

const now = new Date();
const fmtMoeda = (v: number) => formatCurrency(v);

function TrendIcon({ value }: { value: number }) {
  if (Math.abs(value) < 1) return <Minus className="h-3 w-3 text-muted-foreground" />;
  return value > 0
    ? <TrendingUp className="h-3 w-3 text-emerald-600" />
    : <TrendingDown className="h-3 w-3 text-red-600" />;
}

function MiniSparkline({ data }: { data: { value: number }[] }) {
  return (
    <ResponsiveContainer width={80} height={30}>
      <AreaChart data={data} margin={{ top: 2, right: 2, left: 2, bottom: 2 }}>
        <defs>
          <linearGradient id="sparkGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor="hsl(217, 91%, 60%)" stopOpacity={0.3} />
            <stop offset="95%" stopColor="hsl(217, 91%, 60%)" stopOpacity={0} />
          </linearGradient>
        </defs>
        <Tooltip formatter={(v: number) => formatCurrency(v)} />
        <Area type="monotone" dataKey="value" stroke="hsl(217, 91%, 60%)" strokeWidth={1.5} fill="url(#sparkGrad)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}

function VendedorDrilldown({ codVend, vendedor, filters, onClose }: {
  codVend: number; vendedor: string; filters: DashboardFilters; onClose: () => void;
}) {
  const { empresaIds } = useEmpresaContext();
  const { visiveis, multipliers } = useOperacaoFilter();

  const { data: topClientes } = useQuery({
    queryKey: ["vendedor-clientes-drill", codVend, filters.ano, filters.mes, [...visiveis]],
    queryFn: async () => {
      const start = filters.mes ? `${filters.ano}-${String(filters.mes).padStart(2, "0")}-01` : `${filters.ano}-01-01`;
      const end = filters.mes ? new Date(filters.ano, filters.mes, 0).toISOString().split("T")[0] : `${filters.ano}-12-31`;
      
      let q = supabase.from("vendas_union")
        .select("cod_cliente,cliente,uf,cidade,venda,preco_venda,quantidade,operacao,pedido,data")
        .eq("cod_vend", codVend).gte("data", start).lte("data", end);
      if (empresaIds.length > 0) q = q.in("id_empresa", empresaIds);
      
      const allData: any[] = [];
      let offset = 0;
      let hasMore = true;
      while (hasMore) {
        const { data: batch } = await q.range(offset, offset + 999);
        if (batch && batch.length > 0) { allData.push(...batch); offset += 1000; hasMore = batch.length === 1000; }
        else hasMore = false;
      }
      
      const rows = allData.filter(r => visiveis.has(r.operacao));
      const map = new Map<number, { cod: number; cliente: string; uf: string; cidade: string; receita: number; pedidos: Set<number>; ultimaCompra: string }>();
      for (const r of rows) {
        const mult = multipliers.get(r.operacao) ?? 1;
        const receita = (Number(r.venda) || (Number(r.preco_venda) || 0) * (Number(r.quantidade) || 0)) * mult;
        if (!map.has(r.cod_cliente)) map.set(r.cod_cliente, { cod: r.cod_cliente, cliente: r.cliente, uf: r.uf || "", cidade: r.cidade || "", receita: 0, pedidos: new Set(), ultimaCompra: "" });
        const e = map.get(r.cod_cliente)!;
        e.receita += receita;
        if (r.pedido) e.pedidos.add(r.pedido);
        if (r.data > e.ultimaCompra) e.ultimaCompra = r.data;
      }
      
      return [...map.values()].map(c => {
        const diasSemCompra = c.ultimaCompra ? Math.floor((now.getTime() - new Date(c.ultimaCompra).getTime()) / 86400000) : 999;
        return { ...c, qtde_pedidos: c.pedidos.size, dias_sem_compra: diasSemCompra };
      }).sort((a, b) => b.receita - a.receita);
    },
  });

  const cols: Column<any>[] = [
    { key: "cod", label: "Cod" },
    { key: "cliente", label: "Cliente" },
    { key: "uf", label: "UF" },
    { key: "cidade", label: "Cidade" },
    { key: "receita", label: "Receita", align: "right", format: (v) => fmtMoeda(Number(v)) },
    { key: "qtde_pedidos", label: "Pedidos", align: "right" },
    { key: "dias_sem_compra", label: "Dias s/ Compra", align: "right" },
  ];

  return (
    <Dialog open onOpenChange={() => onClose()}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><UserCheck className="h-5 w-5" />{vendedor} — Clientes</DialogTitle>
        </DialogHeader>
        <AdvancedDataTable title="Clientes do Vendedor" columns={cols} data={topClientes || []} pageSize={15} searchKeys={["cliente", "cidade"]} />
      </DialogContent>
    </Dialog>
  );
}

export default function PerformanceVendas() {
  const [filters, setFilters] = useState<DashboardFilters>({ ano: now.getFullYear(), mes: now.getMonth() + 1 });
  const handleFilterChange = useCallback((p: Partial<DashboardFilters>) => setFilters(prev => ({ ...prev, ...p })), []);

  const supervisores = useRankingSupervisores(filters);
  const vendedores = useRankingVendedores(filters);
  const receitaMensal = useReceitaMensal(filters);
  const metasData = useMetasVendas(filters);

  const [expandedSup, setExpandedSup] = useState<string | null>(null);
  const [drillVendedor, setDrillVendedor] = useState<{ codVend: number; vendedor: string } | null>(null);

  const sparkData = (receitaMensal.data || []).map(d => ({ value: d.receita_total }));
  const totalReceita = (supervisores.data || []).reduce((s, d) => s + d.receita_total, 0);

  // Meta lookup
  const supMetas = useMemo(() => {
    const map = new Map<string, number>();
    for (const m of metasData.data?.supervisorMetas || []) map.set(m.referencia_id, m.valor_meta);
    return map;
  }, [metasData.data]);
  const vendMetas = useMemo(() => {
    const map = new Map<string, number>();
    for (const m of metasData.data?.vendedorMetas || []) map.set(m.referencia_id, m.valor_meta);
    return map;
  }, [metasData.data]);

  return (
    <div className="space-y-6 p-4 md:p-6 max-w-[1600px] mx-auto">
      <div className="flex items-center gap-3">
        <div className="flex items-center justify-center h-10 w-10 rounded-lg bg-primary/10">
          <BarChart3 className="h-5 w-5 text-primary" />
        </div>
        <div>
          <h1 className="text-xl font-bold tracking-tight">Performance de Vendas</h1>
          <p className="text-sm text-muted-foreground">Análise por supervisor e vendedor</p>
        </div>
      </div>

      <DashboardFiltersBar filters={filters} onChange={handleFilterChange} />

      {/* Supervisores */}
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            <Users className="h-4 w-4 text-amber-600" />
            Visão por Supervisor
          </CardTitle>
        </CardHeader>
        <CardContent>
          {supervisores.isLoading ? (
            <Skeleton className="h-48 w-full" />
          ) : (
            <div className="space-y-1">
              {(supervisores.data || []).map((sup, idx) => {
                const isExpanded = expandedSup === sup.supervisor;
                const teamVendedores = (vendedores.data || []).filter(v => v.supervisor === sup.supervisor);
                const pctPart = totalReceita > 0 ? (sup.receita_total / totalReceita) * 100 : 0;
                const ticket = sup.qtde_pedidos > 0 ? sup.receita_total / sup.qtde_pedidos : 0;
                const meta = supMetas.get(sup.supervisor);
                const pctMeta = meta && meta > 0 ? (sup.receita_total / meta) * 100 : null;

                return (
                  <div key={sup.supervisor} className="border border-border rounded-lg overflow-hidden">
                    <div
                      className="flex items-center gap-4 p-3 cursor-pointer hover:bg-muted/30 transition-colors"
                      onClick={() => setExpandedSup(isExpanded ? null : sup.supervisor)}
                    >
                      <span className="text-sm text-muted-foreground w-6">{idx + 1}</span>
                      {isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                      <span className="font-medium text-sm flex-1">{sup.supervisor}</span>
                      <div className="hidden md:block"><MiniSparkline data={sparkData} /></div>
                      <div className="text-right min-w-[100px]"><SmartValue value={sup.receita_total} className="text-sm font-semibold" /></div>
                      <div className="text-right min-w-[60px] text-xs text-muted-foreground">{pctPart.toFixed(1)}%</div>
                      <div className="text-right min-w-[80px] text-xs"><SmartValue value={ticket} /></div>
                      {pctMeta !== null && (
                        <div className="min-w-[80px]">
                          <Progress value={Math.min(pctMeta, 100)} className="h-2" />
                          <span className={cn("text-[10px]", pctMeta >= 100 ? "text-emerald-600" : pctMeta >= 80 ? "text-amber-600" : "text-red-600")}>{pctMeta.toFixed(0)}%</span>
                        </div>
                      )}
                    </div>

                    {isExpanded && teamVendedores.length > 0 && (
                      <div className="bg-muted/20 border-t border-border">
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-muted-foreground text-xs">
                              <th className="text-left py-1.5 px-4 font-medium">#</th>
                              <th className="text-left py-1.5 px-2 font-medium">Vendedor</th>
                              <th className="text-right py-1.5 px-2 font-medium">Receita</th>
                              <th className="text-right py-1.5 px-2 font-medium">Pedidos</th>
                              <th className="text-right py-1.5 px-2 font-medium">Clientes</th>
                              <th className="text-right py-1.5 px-2 font-medium">Ticket</th>
                              <th className="text-right py-1.5 px-2 font-medium">% Part.</th>
                              {vendMetas.size > 0 && <th className="text-right py-1.5 px-2 font-medium">Meta</th>}
                              <th className="py-1.5 px-2"></th>
                            </tr>
                          </thead>
                          <tbody>
                            {teamVendedores.map((v, vi) => {
                              const vTicket = v.qtde_pedidos > 0 ? v.receita_total / v.qtde_pedidos : 0;
                              const vPct = totalReceita > 0 ? (v.receita_total / totalReceita) * 100 : 0;
                              const vMeta = vendMetas.get(String(v.cod_vend));
                              const vPctMeta = vMeta && vMeta > 0 ? (v.receita_total / vMeta) * 100 : null;
                              return (
                                <tr
                                  key={v.cod_vend}
                                  className="border-t border-border/30 hover:bg-muted/40 cursor-pointer transition-colors"
                                  onClick={() => setDrillVendedor({ codVend: v.cod_vend, vendedor: v.vendedor })}
                                >
                                  <td className="py-1.5 px-4 text-muted-foreground">{vi + 1}</td>
                                  <td className="py-1.5 px-2">{v.vendedor}</td>
                                  <td className="py-1.5 px-2 text-right"><SmartValue value={v.receita_total} /></td>
                                  <td className="py-1.5 px-2 text-right">{v.qtde_pedidos.toLocaleString("pt-BR")}</td>
                                  <td className="py-1.5 px-2 text-right">{v.clientes_ativos}</td>
                                  <td className="py-1.5 px-2 text-right"><SmartValue value={vTicket} /></td>
                                  <td className="py-1.5 px-2 text-right">{vPct.toFixed(1)}%</td>
                                  {vendMetas.size > 0 && (
                                    <td className="py-1.5 px-2 text-right">
                                      {vPctMeta !== null ? (
                                        <span className={cn("text-xs font-medium", vPctMeta >= 100 ? "text-emerald-600" : vPctMeta >= 80 ? "text-amber-600" : "text-red-600")}>
                                          {vPctMeta.toFixed(0)}%
                                        </span>
                                      ) : "—"}
                                    </td>
                                  )}
                                  <td className="py-1.5 px-2 text-xs text-primary">Detalhar →</td>
                                </tr>
                              );
                            })}
                          </tbody>
                        </table>
                      </div>
                    )}
                  </div>
                );
              })}
            </div>
          )}
        </CardContent>
      </Card>

      {/* Ranking geral vendedores as AdvancedDataTable */}
      <AdvancedDataTable
        title="Ranking Geral de Vendedores"
        columns={[
          { key: "_rank", label: "#", format: (_, row) => String((vendedores.data || []).indexOf(row) + 1) },
          { key: "vendedor", label: "Vendedor" },
          { key: "supervisor", label: "Supervisor" },
          { key: "receita_total", label: "Receita", align: "right", format: (v) => fmtMoeda(Number(v)) },
          { key: "qtde_pedidos", label: "Pedidos", align: "right", format: (v) => Number(v).toLocaleString("pt-BR") },
          { key: "clientes_ativos", label: "Clientes", align: "right" },
          { key: "_ticket", label: "Ticket Médio", align: "right", format: (_, row) => fmtMoeda(row.qtde_pedidos > 0 ? row.receita_total / row.qtde_pedidos : 0) },
          { key: "_pct", label: "% Part.", align: "right", format: (_, row) => totalReceita > 0 ? `${(row.receita_total / totalReceita * 100).toFixed(1)}%` : "0%" },
        ]}
        data={vendedores.data || []}
        pageSize={20}
        searchKeys={["vendedor", "supervisor"]}
      />

      {drillVendedor && (
        <VendedorDrilldown
          codVend={drillVendedor.codVend}
          vendedor={drillVendedor.vendedor}
          filters={filters}
          onClose={() => setDrillVendedor(null)}
        />
      )}
    </div>
  );
}
