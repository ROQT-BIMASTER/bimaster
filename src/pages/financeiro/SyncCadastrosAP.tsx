...
export default function SyncCadastrosAP() {
  ...
  const { data: contasCC, isLoading: ccLoading } = useQuery({
    ...
    queryFn: () => callApi("contas-correntes-api", { path: "/" }),
    staleTime: 60_000,
  });
  ...
  const syncFornMutation = useMutation({
    mutationFn: () => callApi("clientes-api", { path: "/upsert-cpfcnpj", lote: true }),
    onSuccess: () => { toast.success("Fornecedores sincronizados"); qc.invalidateQueries({ queryKey: ["sync-fornecedores"] }); },
    onError: (e: any) => toast.error(e.message),
  });

  const syncCCMutation = useMutation({
    mutationFn: () => callApi("contas-correntes-api", { path: "/upsert-lote", lote: 1, fin_conta_corrente_cadastro: [] }),
    onSuccess: () => { toast.success("Contas Correntes sincronizadas"); qc.invalidateQueries({ queryKey: ["sync-contas-correntes"] }); },
    onError: (e: any) => toast.error(e.message),
  });

  const syncCatMutation = useMutation({
    mutationFn: () => callApi("categorias-api", { path: "/incluir", sincronizar: true }),
    onSuccess: () => { toast.success("Categorias sincronizadas"); qc.invalidateQueries({ queryKey: ["sync-categorias"] }); },
    onError: (e: any) => toast.error(e.message),
  });
