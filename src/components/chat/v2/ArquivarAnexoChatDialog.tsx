/**
 * ArquivarAnexoChatDialog — wizard para arquivar um anexo comum do chat
 * (bucket `chat-anexos`) em um dos 4 cofres oficiais (Submissão China,
 * Briefing, Projeto, Tarefa).
 *
 * Espelha o `VincularDocAprovadoDialog` (que é exclusivo do fluxo de
 * aprovação) mas opera sobre `mensagens_anexos` via novas RPCs
 * `rpc_arquivar_anexo_chat_*`.
 */
...
import { useArquivarAnexoChat, type ArqDestino } from "@/hooks/chat/useArquivarAnexoChat";
...
export function ArquivarAnexoChatDialog({ open, onOpenChange, anexo }: Props) {
  ...
  const [destino, setDestino] = useState<ArqDestino | null>(null);
  ...
  const arq = useArquivarAnexoChat();
  ...
  const submissoes = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.rpc as any)("rpc_chat_vinculo_submissoes_china");
      if (error) throw error;
      return (data ?? []) as { id: string; produto_codigo: string | null; produto_nome: string | null; status: string | null }[];
    },
  });
  ...
  const projetos = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.rpc as any)("get_accessible_projetos", {
        _target_user_id: null, _include_all: false,
      });
      if (error) throw error;
      return (data ?? []).map((p: any) => ({ id: p.id, nome: p.nome })) as { id: string; nome: string }[];
    },
  });
  ...
  const tarefas = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.rpc as any)("rpc_chat_vinculo_tarefas_projeto", { p_projeto_id: projetoId });
      if (error) throw error;
  ...
  const tiposChinaDisp = useMemo(() => {
    ...
    if (!cat) return [];
    return cat.tipos.filter((t) => !checklist.hiddenSet.has(t))
  ...
  const base = {
    anexo_id: anexo.id,
    storage_path_origem: anexo.storage_path,
    nome_arquivo: anexo.file_name,
    mime_type: anexo.mime_type,
    size_bytes: anexo.size_bytes,
  };

  const isPending =
    arq.arquivarChina.isPending || arq.arquivarBriefing.isPending ||
    arq.arquivarProjeto.isPending || arq.arquivarTarefa.isPending;
  ...
  const submit = async () => {
    ...
    if (destino === "china_checklist") {
      arq.arquivarChina.mutate({ ...base, submissao_id: submissaoId, tipo_documento: tipoChina }, { onSuccess: close });
    } else if (destino === "briefing") {
      arq.arquivarBriefing.mutate({ ...base, briefing_id: briefingId, categoria: categoriaBrief }, { onSuccess: close });
    } else if (destino === "projeto") {
      arq.arquivarProjeto.mutate({ ...base, projeto_id: projetoId, categoria: categoriaProj }, { onSuccess: close });
    } else if (destino === "tarefa") {
      arq.arquivarTarefa.mutate({ ...base, projeto_id: projetoId, tarefa_id: tarefaId, categoria: categoriaProj }, { onSuccess: close });
    }
  };

  const destinos: { key: ArqDestino; label: string; icon: any; enabled: boolean }[] = [
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Arquivar no Cofre Oficial</DialogTitle>
          <DialogDescription className="flex items-center gap-1.5 text-xs">
            <FileText className="h-3.5 w-3.5" /> {anexo.file_name}
          </DialogDescription>
        </DialogHeader>
