...
export function ContasPagarTabContent({ filterEmpresas, filterAno, filterMes, filterDepartamento, filterPortadores, filterDiaVencimento = "", filterDiaPagamento = "", filterConta = "all", filterNatureza = "all", filterCentroCusto = "all", filterPlanoContas = "all" }: Props) {
  ...
  const { data: empresas } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase as any).from("empresas").select("id,nome").eq("ativa", true).order("nome");
      return data || [];
    },
  });
                  ...
                  <TableBody>
                    {paginated.map(c => (
                      <TableRow key={c.id}>
                        ...
                        <TableCell className={cn("text-sm", calculateFinancialStatus(c.data_vencimento, c.data_pagamento, c.status, c.valor_aberto, c.valor_pago) === "vencido" && "text-destructive font-semibold")}>
