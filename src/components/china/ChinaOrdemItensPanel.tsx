...
const STATUS_CFG: Record<string, { pt: string; cn: string; variant: any; bar: string }> = {
  aberto: { pt: "Aberto", cn: "未开始", variant: "secondary", bar: "border-l-muted-foreground/40" },
  parcial: { pt: "Parcial", cn: "部分", variant: "warning", bar: "border-l-warning" },
  fechado: { pt: "Concluído", cn: "已完成", variant: "success", bar: "border-l-success" },
  cancelado: { pt: "Cancelado", cn: "已取消", variant: "destructive", bar: "border-l-destructive" },
};
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <BilingualLabel pt="Itens & Saldo" cn="项目与余额" size="md" />
          </div>
          <div className="flex items-center gap-3 text-xs">
            <span className="text-muted-foreground">
              Total: <strong className="text-foreground">{totalPedido.toLocaleString()}</strong>
            </span>
            <span className="text-muted-foreground">
              Saldo: <strong className={saldoTotal > 0 ? "text-warning" : "text-success"}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-center text-xs">
          ...
          <div className="p-2 bg-blue-500/10 rounded-lg">
            <p className="text-muted-foreground">Embarcado</p>
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-sm">{item.cor_nome || item.sku || "Único"}</p>
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-muted-foreground w-16">Produção</span>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-muted-foreground w-16">Recebimento</span>
                ...
                {item.qty_cancelada > 0 && (
                  <p className="text-[10px] text-destructive mt-2">
                    {item.qty_cancelada} unidades canceladas
                  </p>
                )}
              </div>
            );
