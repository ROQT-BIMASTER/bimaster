...
import { useComposicaoErpSync } from '@/hooks/useComposicaoErpSync';
import {
  RefreshCw, Database, Clock, CheckCircle, XCircle, Loader2,
  Calendar, Activity, Server, Play, Timer, FlaskConical, Layers, Boxes,
} from 'lucide-react';
import { formatDistanceToNow, format } from 'date-fns';
import { ptBR } from 'date-fns/locale';

export function ComposicaoErpSyncPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold">Sincronização — Composição de Produto</h2>
          <p className="text-sm text-muted-foreground">
            Engine direta SQL Server (ComposicaoProduto) → Banco de dados
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><Layers className="h-4 w-4 text-primary" />Total de Registros</CardTitle></CardHeader>
          <CardContent>
            <p className="text-3xl font-bold">{stats?.totalRegistros.toLocaleString() || '0'}</p>
            <p className="text-sm text-muted-foreground">linhas de composição</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><Database className="h-4 w-4" />Empresas</CardTitle></CardHeader>
          <CardContent>
            <p className="text-3xl font-bold">{stats?.empresasDistintas.toLocaleString() || '0'}</p>
            <p className="text-sm text-muted-foreground">com composição</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><Boxes className="h-4 w-4 text-green-500" />Produtos</CardTitle></CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-green-600">{stats?.produtosDistintos.toLocaleString() || '0'}</p>
            <p className="text-sm text-muted-foreground">distintos</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><FlaskConical className="h-4 w-4 text-cyan-500" />Matérias-Primas</CardTitle></CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-cyan-600">{stats?.materiasDistintas.toLocaleString() || '0'}</p>
            <p className="text-sm text-muted-foreground">distintas</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Sync direta via SQL Server — paginada por empresa (Empresa_Compo)</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex gap-2">
              <Input placeholder="ID Empresa (Empresa_Compo)" value={empresaId} onChange={e => setEmpresaId(e.target.value)} />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Últimas execuções via ERP Sync Engine — Composição</CardDescription>
        </CardHeader>
        <CardContent>
