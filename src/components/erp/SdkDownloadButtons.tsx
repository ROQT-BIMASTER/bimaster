...
function sdkHeader(lang: string): string {
  ...
  return [
    ...
    `${comment}   - ETAG / IF-NONE-MATCH (RFC 7232): GETs cacheáveis (/query, /consultar, /status) agora`,
    ...
    `${comment} Changelog v2.12.0:`,
    `${comment}   - Paridade total restaurada (60/60/60): Python e JS ganharam cp_parcelas_sync,`,
    `${comment}     cp_anexos_listar, cp_anexos_incluir, cp_cancelar_lote (cobertura CP 19/19)`,
    `${comment} Changelog v2.11.0:`,
    `${comment}   - PARIDADE TS COMPLETA: crQuery/crGetRecebimentos/crGetParcelas tipados`,
    `${comment}   - COBERTURA CP 19/19: cpParcelasSync, cpAnexosListar, cpAnexosIncluir, cpCancelarLote (TS)`,
    `${comment} Changelog v2.10.0:`,
    `${comment}   - Python: cp_query valida chaves desconhecidas (paridade com TS/JS v2.9.0)`,
    `${comment}   - Edge Function erp-export-payment validada ao vivo (400 estruturado)`,
    `${comment} Changelog v2.9.0:`,
    `${comment}   - DX: erp-export-payment retorna 400 estruturado (validation_error + details + request_id)`,
    `${comment}   - TS: crConsultar tipado (CrConsultarResponse) — paridade com CpConsultarResponse`,
    `${comment} Changelog v2.8.0: PARIDADE CR/CP — retry/idempotencyKey + família moderna CR`,
    `${comment} Changelog v2.7.0: Retry idempotente promovido à API pública dos endpoints CP`,
